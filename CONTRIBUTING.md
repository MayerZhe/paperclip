...
2. **No issue exists** — describe the problem directly in your PR body, following one of our [issue templates](.github/ISSUE_TEMPLATE/) so a reviewer has the same fields they'd get from a filed issue:
   - **Bug fix:** what happened, expected behavior, steps to reproduce, Super Node version/commit, deployment mode. See [`bug_report.yml`](.github/ISSUE_TEMPLATE/bug_report.yml).
...
### Super Node Gates Must Pass

All Super Node CI gates (lint, typecheck, tests, build, and any other required checks) must be satisfied before a PR can be merged. Don't ask for a merge while gates are red — fix them first.
...
We actively manage the core Super Node feature roadmap.
...
> - Super Node is the open source app people use to manage AI agents for work
...
> - That way we can get optimal performance from any adapter/provider in Super Node
