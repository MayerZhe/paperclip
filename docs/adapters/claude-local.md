...
The adapter creates a temporary directory with symlinks to Super Node skills and passes it via `--add-dir`. This makes skills discoverable without polluting the agent's working directory.
...
This installs Super Node skills in `~/.claude/skills`, creates an agent API key, and prints shell exports to run as that agent.
