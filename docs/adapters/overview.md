...
summary: What adapters are and how they connect agents to Super Node
---

Adapters are the bridge between Super Node's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Super Node:
...
You can build and distribute adapters as standalone packages — no changes to Super Node's source code required. External adapters are loaded at startup via the plugin system.
...
External adapters can ship a self-contained UI parser that tells the Super Node web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
