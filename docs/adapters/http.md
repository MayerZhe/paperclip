...
The `http` adapter sends a webhook request to an external agent service. The agent runs externally and Super Node just triggers it.
...
1. Super Node sends a POST request to the configured URL
2. The request body includes the execution context (agent ID, task info, wake reason)
3. The external agent processes the request and calls back to the Super Node API
...
The external agent uses `PAPERCLIP_API_URL` and an API key to call back to Super Node.
