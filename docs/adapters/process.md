...
- Running a Python script that calls the Super Node API
...
1. Super Node spawns the configured command as a child process
2. Standard Super Node environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Super Node API and perform work.
