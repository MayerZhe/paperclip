...
| Distribution | Ships with Super Node | Published to npm or linked via `file:` |
| Updates | Requires Super Node release | Independent versioning |
...
Local adapters are symlinked into Super Node's adapter directory. Changes to the source are picked up on server restart.
...
Other Super Node users can then install your adapter by package name from the UI or API.
