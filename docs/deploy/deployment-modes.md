...
Super Node supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
When migrating from `local_trusted` to `authenticated`, Super Node emits a one-time claim URL at startup:
