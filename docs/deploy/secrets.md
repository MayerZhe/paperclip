...
Super Node encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Super Node protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Super Node does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Super Node can no longer guarantee
...
At runtime, Super Node resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Super Node injects its
...
Super Node best-effort enforces `0600` permissions when it creates or loads the
...
Provider-owned secrets can be linked without copying values into Super Node by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Super Node stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Super Node Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Super Node company secrets**. Allowed credential sources are
workload identity attached to the Super Node server (instance profile, IRSA, ECS
...
Super Node `external_reference` secrets. This is a metadata-only link: Super Node
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the Super Node name and key, then
   submit.
...
cursors. Do not expect Super Node to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Super Node runtime
...
- Keep managed create/rotate/delete permissions scoped to the Super Node
...
dedicated Super Node runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Super Node
  ...
  key bytes.
- `aws_secrets_manager`: back up Super Node's database for vault metadata
  ...
  configured prefix; restore by pointing the same Super Node company at the
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Super Node. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Super Node
...
For Super Node Cloud, provision the server runtime IAM role/workload identity,
...
Super Node secrets. Bootstrap material belongs in infrastructure IAM/workload
...
Use the CLI command for normal operations because it goes through the Super Node
...
the value custodian; Super Node stores metadata and provider version references,
