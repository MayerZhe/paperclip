...
summary: Run Super Node with Tailscale-friendly bind presets and connect from other devices
---

Use this when you want to access Super Node over Tailscale (or a private LAN/VPN) instead of only `localhost`.

## 1. Start Super Node in private authenticated mode
...
From the machine running Super Node:
...
## 3. Open Super Node from another device

Use the Tailscale IP or MagicDNS host with the Super Node port:
...
If you access Super Node with a custom private hostname, add it to the allowlist:
