...
Audience: Operators setting up and running agents in Super Node

## 1. What this system does

Agents in Super Node do not run continuously.  
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Super Node assumes the CLI is already installed and authenticated on the host machine.
...
Super Node stores session IDs for resumable adapters.
...
Super Node pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Super Node surfaces this as a warning in environment tests, not a hard error.
