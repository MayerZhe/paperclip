...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Super Node saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
Exports are full by default. `traces/` keeps the Super Node envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.
...
- Default endpoint: when no feedback export backend URL is configured, Super Node falls back to `https://telemetry.paperclip.ing`
