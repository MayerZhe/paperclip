...
Add first-class **documents** to Super Node as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Super Node workflow/docs.
...
Update the Super Node agent workflow so planning no longer edits the issue description.
