...
If Super Node is already configured, rerunning `onboard` keeps the existing config in place. Use `paperclipai configure` to change settings on an existing install.
...
On an existing install, `--yes` now preserves the current config and just starts Super Node with that setup.
