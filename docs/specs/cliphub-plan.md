# ClipHub: Marketplace for Super Node Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Super Node configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Super Node-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Super Node company (agents, projects, governance) |
...
A complete Super Node company configuration:
...
Single-agent configurations designed to plug into a Super Node org:
...
Portable skill files that any Super Node agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Super Node's skill loading system
...
| `POST` | `/api/listings/:id/install` | Install to Super Node company |
...
### 5.3 Creator: Export from Super Node → Publish
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Super Node brand blue/purple)
...
| Install flow | One-click deploy to Super Node company |
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Super Node
- **Backend**: Node.js API (or extend Super Node server)
- **Database**: Postgres (can share Super Node's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Super Node auth (or OAuth2)

### 9.2 Integration with Super Node

ClipHub can be:
- **Option A**: A separate app that calls Super Node's API to install blueprints
- **Option B**: A built-in section of the Super Node UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Super Node UI and API.
...
- [ ] Install flow: blueprint → Super Node company
...
- [ ] "Export from Super Node" CLI command
