...
Super Node is a monorepo with four main layers.
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Super Node env vars and a prompt
4. **Agent work** — The agent calls Super Node's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Super Node and agent runtimes. Each adapter is a package with three modules:
...
- **Control plane, not execution plane** — Super Node orchestrates agents; it doesn't run them
