...
Super Node `external_reference` secrets without copying plaintext into
Super Node.
...
`ListSecrets` only and import stores a Super Node external reference plus
...
remote metadata, or copies secret plaintext into Super Node.
...
- `conflict`: the suggested Super Node `name` or `key` is already in use.
...
Super Node-managed AWS secret through a broad runtime role.
...
Each row may include an optional Super Node `description` entered during review;
...
the Super Node runtime role can list the AWS secret but lacks
