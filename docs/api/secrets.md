...
To link a provider-owned secret without copying the value into Super Node, create
...
Super Node stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in Super Node `company_secrets`.
...
Remote import links existing AWS Secrets Manager entries into Super Node as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into Super Node.
...
- `duplicate`: a Super Node secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under Super Node's own managed namespace are
blocked as external references; use the Super Node-managed secret flow for those
resources instead.
...
Super Node `name`, `key`, optional Super Node `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into Super Node
...
- `imported`: Super Node created an active `external_reference` secret and one
...
real value into the agent process environment. Super Node's custody guarantees
