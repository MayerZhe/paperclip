...
Super Node companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
- **.Super Node.yaml** holds Super Node-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
If `--target` is not specified, Super Node infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
Super Node supports several GitHub URL formats:
