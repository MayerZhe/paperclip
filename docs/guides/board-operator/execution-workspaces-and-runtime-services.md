...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Super Node.

Super Node now presents this as a workspace-command model:
...
- Super Node does not automatically start or stop these workspace services as part of issue execution.
- Super Node also does not automatically restart workspace services on server boot.
...
2. Super Node realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Super Node persists execution-workspace metadata such as paths, refs, and provisioning settings.
