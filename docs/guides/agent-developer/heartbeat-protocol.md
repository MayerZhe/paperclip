...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Super Node.
...
Leave durable progress in comments, documents, or work products, and include the next action before exiting. For parallel or long delegated work, create child issues and let Super Node wake the parent when they complete instead of polling agents, sessions, or processes.
...
Super Node records run liveness as metadata on heartbeat runs. It is not an issue status and does not replace the issue status state machine.
...
- Continuations do not mark the issue `blocked` or `done`. If automatic continuations are exhausted, Super Node leaves an audit comment so a human or manager can clarify, block, or assign follow-up work.
