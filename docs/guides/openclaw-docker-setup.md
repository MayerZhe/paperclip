...
How to get OpenClaw running in a Docker container for local development and testing the Super Node OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Super Node includes an end-to-end join smoke harness:
...
- probes and prints a Super Node host URL that is reachable from inside OpenClaw Docker
...
If your Super Node deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Super Node server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Super Node URL (typically `http://host.docker.internal:3100`).
- If Super Node rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm paperclipai allowed-hostname host.docker.internal
```

Then restart Super Node and rerun the smoke script.
