...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Super Node.
...
- Super Node must not require extra top-level fields for skill validity
- Super Node-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
For Super Node, the preferred fidelity extension is:
...
- issue/task Super Node-only metadata
...
- Super Node tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Super Node shape:
...
Additional rules for Super Node exporters:
...
- omit empty and default-valued Super Node fields when possible
...
- Super Node exporters should emit `.paperclip.yaml` as a sidecar by default
...
## 19. Super Node Mapping

Super Node can map this spec to its runtime model like this:
...
- Super Node extension:
  - `.paperclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Super Node-specific fidelity

Inline Super Node-only metadata that must live inside a shared markdown file should use:

- `metadata.paperclip`

That keeps the base format broader than Super Node.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Super Node.

## 20. Cutover

Super Node should cut over to this markdown-first package model as the primary portability format.

`paperclip.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Super Node, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
