...
- Super Node issue context now has incremental APIs: `GET /api/agents/me/inbox-lite`, `GET /api/issues/:id/heartbeat-context`, and comment delta queries via `GET /api/issues/:id/comments?after=...&order=asc`.
...
- `codex_local` currently injects Super Node skills into the shared Codex skills home (`$CODEX_HOME/skills` or `~/.codex/skills`) rather than mounting a worktree-local skill directory.
- If a Super Node-owned skill symlink already points at another live checkout, the adapter currently skips it instead of repointing it.
