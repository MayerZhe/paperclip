...
function awsSecretsManagerCheck(): CheckResult {
  ...
  if (missingConfig.length > 0) {
    return {
      ...
      canRepair: false,
      repairHint:
        `Set ${missingConfig.join(", ")} in the Super Node server runtime. ${AWS_CREDENTIAL_SOURCE_HINT}. Do not store AWS root credentials or long-lived IAM user keys in Super Node secrets.`,
    };
  }
  ...
  if (staticEnvCredentials) {
    return {
      ...
      canRepair: false,
      repairHint:
        "AWS static environment credentials are visible. Use only short-lived shell credentials locally; prefer IAM role/workload identity for hosted deployments and never store AWS access keys in Super Node company secrets.",
    };
  }
