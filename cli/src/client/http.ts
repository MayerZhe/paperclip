...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Super Node API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Super Node server is not running, the configured URL is wrong, or the request is being blocked before it reaches Super Node.",
    "",
    "Try:",
    "- Start Super Node with `pnpm dev` or `pnpm paperclipai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Super Node is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
