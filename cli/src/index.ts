...
const DATA_DIR_OPTION_HELP =
  "Super Node data directory root (isolates state from ~/.paperclip)";

program
  .name("paperclipai")
  .description("Super Node CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program
  ...
  .option("--run", "Start Super Node immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Super Node setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
const run = program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Super Node")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Super Node server API")
