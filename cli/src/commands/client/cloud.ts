...
export function registerCloudCommands(program: Command): void {
  const cloud = program.command("cloud").description("Super Node Cloud upstream sync commands");

  addCommonClientOptions(
    cloud
      .command("connect")
      .description("Authorize this local instance to push into a Super Node Cloud stack")
      .argument("<remote-url>", "Super Node Cloud stack URL")
  ...
  addCommonClientOptions(
    cloud
      .command("push")
      .description("Preview or apply a local company push into the connected Super Node Cloud stack")
...
export async function connectCloud(remoteUrl: string, opts: CloudConnectOptions = {}): Promise<CloudConnection> {
  ...
  if (ctx.json) {
    ...
    console.log(pc.bold("Connected to Super Node Cloud"));
...
export function assertDiscoveryCompatible(discovery: UpstreamDiscovery): void {
  if (discovery.schema !== "paperclip-upstream-discovery-v1") {
    throw new Error("Remote URL is not a Super Node Cloud upstream target.");
  }
  ...
  if (!discovery.transfer.featureFlags?.includes("cloud_sync")) {
    throw new Error("Remote Super Node Cloud stack does not advertise the cloud_sync transfer flag.");
  }
}
...
async function authorizeConnection(
  ...
  if (!discovery.auth.deviceCode) {
    throw new Error("Remote Super Node Cloud stack does not support device-code authorization.");
  }
...
async function startPkceCallbackServer(): Promise<{
  ...
  const server = createServer((req, res) => {
    ...
    if (!code || state !== expectedState) {
      ...
      res.end("Super Node Cloud authorization failed. You can close this tab.");
      rejectCode?.(new Error("Authorization callback was missing a valid code or state."));
      return;
    }
    res.writeHead(200, { "Content-Type": "text/plain" });
    res.end("Super Node Cloud authorization complete. You can close this tab.");
    resolveCode?.(code);
  });
