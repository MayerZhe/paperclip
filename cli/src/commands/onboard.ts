...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Super Node install detected; keeping the current configuration unchanged."),
    );
    ...
    p.outro("Existing Super Node setup is ready.");
    return;
  }
