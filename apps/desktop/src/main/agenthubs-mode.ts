...
export interface StartAgentHubsConfig {
  ...
  /** JWT token for AgentHubs Cloud authentication (SN registration) */
  agenthubsToken?: string;
  /** Org ID on AgentHubs Cloud (SN registration) */
...
/**
 * Wait for SnFeedClient WebSocket to connect, polling every 200ms.
 * Throws if the client enters error state or times out.
 */
...
export async function startAgentHubsMode(
  ...
  // 8. Set security policy — allow networking + inject token for cloud auth
  onProgress?.("configuring VM security policy...");
  try {
    const policyParams: Record<string, unknown> = { allowNetwork: true };
    if (config.agenthubsToken) {
      policyParams.token = config.agenthubsToken;
    }
    if (config.agenthubsOrgId) {
      policyParams.orgId = config.agenthubsOrgId;
    }
    await rpc.request("SetSecurityPolicy", policyParams);
  } catch (err) {
    const errorMsg = err instanceof Error ? err.message : String(err);
  ...
  // 11. Register SN via SnFeedClient (if token and orgId are configured)
  // Non-blocking: failure to register SN does NOT prevent the VM from running.
    ...
    try {
      ...
      const feedClient = new SnFeedClient({
        ...
        onOrder: (order: SnOrder) => {
          console.log(`[AgentHubs] New order received: ${order.id} — ${order.title}`);
        },
        onError: (error: Error) => {
      ...
      console.warn(`[AgentHubs] SN registration failed (non-fatal): ${errMsg}`);
      // Don't re-throw — SN registration is not critical for VM operation
    }
  }
...
export async function stopAgentHubsMode(
  ...
  // 1b. Stop SnFeedClient (if active)
  if (snFeedClient) {
    try {
      snFeedClient.stop();
    } catch (err) {
      console.warn(`[AgentHubs] Failed to stop SnFeedClient: ${err}`);
    }
    snFeedClient = null;
  }
