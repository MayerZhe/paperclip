...
required: false
description: >
  Develop and operate a local Super Node instance — start and stop servers,
  ...
  Super Node codebase itself or keep a running instance healthy.
---

# Super Node Dev

This skill covers the day-to-day workflows for developing and operating a local Super Node instance. It assumes you are working inside the Super Node repo checkout with `origin` pointing to `git@github.com:paperclipai/paperclip.git`.
...
> **MANDATORY:** Before running any CLI command, building, testing, or managing worktrees, you MUST read `doc/DEVELOPING.md` in the Super Node repo. It is the canonical reference for all `paperclipai` CLI commands, their options, build/test workflows, database operations, worktree management, and diagnostics. Do NOT guess at flags or options — read the doc first.
...
Super Node worktrees combine git worktrees with isolated Super Node instances — each gets its own database, server port, and environment seeded from the primary instance.
...
- Starting a feature branch that needs its own Super Node environment
- Running parallel agent work without cross-contaminating the primary instance
- Testing Super Node changes in isolation before merging
...
| `worktree:list` | List worktrees and their Super Node status |
...
# 3. Start the isolated Super Node server
npx paperclipai run
