---
name: task-planning
description: Turn a Super Node issue or request into a structured implementation plan with child task graph, blockers, owners, and acceptance criteria, then save it as the issue `plan` document.
...
Produce implementation plans that the Super Node executor can actually run: explicit child issues, real blockers, named owners, and a defined acceptance bar. Avoid plans that read well but cannot be split into work.
...
Use the Super Node API to write the plan document, then comment:
...
When the plan is accepted, see the companion skill for converting accepted plans into Super Node executable tasks.
