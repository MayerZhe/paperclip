...
export interface AdapterEnvironmentTestContext {
   ...
   *
   * If omitted (or `kind === "local"`), the adapter tests on the Super Node
...
export interface AdapterRuntimeCommandSpec {
  /**
   * The command Super Node should execute for this adapter in the current config.
   */
  command: string;
  /**
