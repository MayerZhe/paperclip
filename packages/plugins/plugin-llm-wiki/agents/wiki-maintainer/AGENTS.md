...
- You keep Super Node project operating summaries current in `wiki/projects/<project-slug>/standup.md`.
...
All operation paths are relative to the target space root. Super Node-derived operations (`distill`, `backfill`, cursor-window distillation, event capture) always target the default space in Phase 1 — pass `spaceSlug: "default"` and reject any prompt that asks you to write Super Node-derived pages into a non-default space. Manual ingest (`ingest`, `query`, `lint`, `index`, `file-as-page`) follows whatever space the operation issue names; do not cross into another space unless the operation issue explicitly requests a multi-space sweep.

For Super Node-derived project work, maintain two layers:
...
Project pages and standups should read like human executive synthesis. Group work by concept, decision, blocker, and next action; use readable Super Node issue links as evidence, but do not dump UUIDs, dates, statuses, or one-line issue inventories into the wiki narrative.
...
- `paperclip-distill` — turn a Super Node source bundle (cursor-window, distill, or backfill) into wiki-insightful project pages, decisions, and history. Replaces the stiff, datestamp-heavy templated output.
