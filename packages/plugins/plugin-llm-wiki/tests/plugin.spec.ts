...
function paperclipIssue(overrides: Partial<Issue> = {}): Issue {
  ...
  return {
    ...
    description: "Decide which Super Node issues, comments, and documents can be ingested into the wiki.",
    status: "todo",
    workMode: "standard",
    priority: "medium",
...
`;
  ...
  it("does not ingest Super Node events until operator controls enable them", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("records enabled Super Node issue events as cursor observations without creating ingest operations", async () => {
    const harness = createTestHarness({ manifest });
    const issue = paperclipIssue();
    harness.seed({ issues: [issue] });
    const writes: Array<{ path: string; contents: string }> = [];
  ...
  it("preserves Super Node event ingestion sources when only enabled changes", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("keeps Super Node event cursor observations company scoped and ignores plugin-operation issues", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("routes Super Node issue, comment, and document event cursors only to the default space", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("routes Super Node events into an explicitly enabled shared non-default space", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("assembles deterministic Super Node source bundles with issue, document, and comment provenance", async () => {
    const harness = createTestHarness({ manifest });
  ...
  it("records estimated Super Node distillation cost without refusing on legacy cost config", async () => {
    const harness = createTestHarness({
      manifest,
      config: {
        maxSuper NodeRoutineRunCostCents: 1,
        maxSuper NodeDistillationTaskCostCents: 1,
        maxSuper NodeDistillationProjectCostCents: 1,
  ...
  it("backfills only the selected Super Node project and date window", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
  ...
  it("generates review-required Super Node project page patches with provenance, index, and log updates", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = paperclipIssue({
  ...
  it("auto-applies Super Node project page patches by default when policy allows and records page bindings", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = paperclipIssue({
  ...
  it("refuses auto-apply Super Node project page patches in authenticated/public deployments", async () => {
  ...
  it("refuses stale project page hashes before writing generated Super Node pages", async () => {
    const harness = createTestHarness({ manifest, config: { autoApplyIngestPatches: true } });
    const project = existingProject();
    const issue = paperclipIssue({
  ...
  it("skips low-signal Super Node source windows without proposing wiki writes", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = paperclipIssue({
  ...
  it("preserves manual source ingest for non-default spaces while refusing Super Node distillation there", async () => {
    const harness = createTestHarness({ manifest });
    const writes: Array<{ path: string; contents: string }> = [];
    harness.ctx.localFolders.writeTextAtomic = async (_companyId, _folderKey, relativePath, contents) => {
      writes.push({ path: relativePath, contents });
    ...
    })).rejects.toThrow("Super Node ingestion policy denied queue");
  });

  it("fails closed for direct Super Node ingestion actions against restricted spaces", async () => {
    const harness = createTestHarness({ manifest });
    harness.seed({
      agents: [wikiMaintainerAgent()],
      projects: [existingProject()],
  ...
  it("re-checks Super Node ingestion policy at execution time for queued work", async () => {
    const harness = createTestHarness({ manifest });
    const project = existingProject();
    const issue = paperclipIssue({
    ...
    await expect(harness.performAction("create-paperclip-distillation-run", {
      companyId: COMPANY_ID,
      projectId: project.id,
    })).rejects.toThrow("personal spaces cannot ingest Super Node sources");
    expect(harness.dbExecutes.some((execute) =>
      execute.sql.includes("paperclip_distillation_runs") && execute.sql.includes("'source_ready'"))).toBe(false);
  });

  it("queues Super Node ingestion backfills for every selected project scope", async () => {
    const harness = createTestHarness({ manifest });
    await plugin.definition.setup(harness.ctx);
  ...
  it("rejects oversized Super Node ingestion profile and source-scope payloads", async () => {
    const harness = createTestHarness({ manifest });
    ...
    await expect(harness.performAction("update-event-ingestion-settings", {
      ...
      maxCharacters: 20001,
      sources: { issues: true },
    })).rejects.toThrow("maxCharacters exceeds the hard Super Node ingestion cap");
