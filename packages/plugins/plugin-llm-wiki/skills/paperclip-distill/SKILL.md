...
description: Use when an operation issue is a Super Node cursor-window, distill, or backfill — `operationType: "distill"` or `"backfill"` and the body references a Super Node source bundle for a project or root issue. Turn raw Super Node activity into a wiki-insightful project page, decisions log, and history note. This skill exists specifically to replace the stiff, datestamp-heavy templated output that the deterministic distiller produces.
---

# Super Node Distill

Distill Super Node project, issue, comment, and document activity into durable wiki pages. The success criterion is **wiki-insightful, not procedural**: a reader who has never seen Super Node should learn what the project is, what was decided, what is at risk, and what the current state is — without scanning a list of `## [YYYY-MM-DD]` headers.

## When this skill is needed

- Cursor-window distillation: the routine fed you a bounded source bundle of recent Super Node activity for one project or root issue.
...
In Phase 1, every Super Node distill, backfill, and cursor-window operation writes into the
...
write Super Node-derived pages into a non-default space.

This rule is destination-only. The Super Node source scope (which projects, root issues,
comments, documents are read) is set elsewhere in the operation issue and is independent of
the destination.

## Inputs

- A Super Node source bundle (issue list, comment refs, document refs, source hash, cursor window).
...
## Super Node Asset Gate

Do not treat Super Node assets/attachments or issue work products as source text for this skill.

- Allowed Super Node body text: issue descriptions, comment bodies, document bodies.
...
2. **Procedural status lists.** `Issue mix: 3 todo, 5 in_progress, 2 done` tells the reader nothing they could not read off Super Node directly. State *what is happening and why it matters*, then cite the issues that constitute the evidence.
...
4. **Write `wiki/projects/<slug>/standup.md` first.** Every Super Node project represented in the wiki must have this file. It is the executive standup: where the project stands today, what changed recently, what is blocked or risky, and what happens next. Use stable sections, in this order:
   ...
   - **Next Actions** — concrete next actions and owners inferred from Super Node issues, not vague aspirations.
   - **Links** — durable wiki project page and relevant Super Node project/issues/documents.
   Rewrite the standup to today's state. Do not append endless dated sections; the audit trail belongs in `wiki/log.md` and Super Node comments.
5. **Write `wiki/projects/<slug>/index.md`** with these stable sections, in this order:
   ...
   - **References** — readable links to the current standup and supporting Super Node tasks/documents. Keep hashes and cursor ids out of the narrative.
...
- Cite Super Node source refs inline using their issue identifier (e.g. `PAP-3179`), not opaque UUIDs.
...
- [ ] The project page reads as wiki content, not as a Super Node status report. A reader new to the company should understand what the project is.
...
`wiki_search`, `wiki_read_page`, `wiki_write_page`, `wiki_list_sources`, `wiki_read_source`. Always include the operation issue's `wikiId` and `spaceSlug`. The Super Node source bundle arrives as part of the operation context — you do not need to assemble it.
