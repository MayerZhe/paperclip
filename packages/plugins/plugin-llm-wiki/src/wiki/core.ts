...
function assertPaperclipSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("Super Node source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard Super Node ingestion cap of ${max} characters.`);
  }
}
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original Super Node source directly if a human needs the unredacted material.",
    ].join("\n"),
...
function evaluatePaperclipProfilePolicy(input: {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `Super Node ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `Super Node ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest Super Node sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `Super Node ingestion policy denied ${purpose}: Super Node ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `Super Node ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
...
async function validatePaperclipIngestionProfile(ctx: PluginContext, input: {
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("Super Node ingestion profile must include at least one source scope before it can be enabled.");
  }
...
export async function updatePaperclipIngestionProfile(ctx: PluginContext, input: {
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated Super Node ingestion profile for ${space.displayName}`,
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  ...
  return [
    ...
    paperclipDerived
      ? "- Super Node-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write Super Node-derived pages into a non-default space."
...
function sourceRefLabel(ref: PaperclipSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "Super Node source");
...
function standupPageContents(input: {
  ...
  const title = input.project?.name ?? input.rootIssue?.title ?? "Super Node Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful Super Node work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes Super Node issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify Super Node before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
export async function distillPaperclipProjectPage(ctx: PluginContext, input: PaperclipProjectPageDistillationInput) {
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordPaperclipDistillationOutcome(ctx, {
      ...
      warning: "Skipped low-signal Super Node source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal Super Node source window."],
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordPaperclipDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged Super Node source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged Super Node source hash."],
      patches: [] as PaperclipDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "Super Node Project";
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `Super Node distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Super Node issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Super Node comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# Super Node document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
