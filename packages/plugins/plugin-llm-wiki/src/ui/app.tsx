        ...
        <div style={{ padding: "16px 18px", borderBottom: `1px solid ${tokens.border}`, display: "flex", gap: 12, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <Tiny style={{ marginTop: 4 }}>
              ...
              Wiki ingest operations. This is manual file ingest - Super Node-derived distillation always
    ...
    >
      ...
      >
        <div style={{ padding: "16px 20px", borderBottom: `1px solid ${tokens.border}`, display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <Tiny style={{ marginTop: 4 }}>
              ...
              under the wiki root. Super Node distillation and event capture always write into the
              default space and skip new spaces created here - per-space Super Node routing is a later
              phase.
            </Tiny>
          </div>
        ...
        <div style={{ padding: 20, display: "grid", gap: 16 }}>
          ...
          <FormField label="Access scope" help="Access scope is metadata only. It does not currently enforce who can read or write the space, and it does not change which Super Node sources reach the space.">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto border-t border-border px-3 py-3">
        ...
        <div className="mb-1 flex items-center gap-1 px-2 text-[11px] font-semibold uppercase tracking-normal text-muted-foreground" style={{ height: 24 }}>
          <span
            ...
            title="Destination spaces. Browsing and manual ingest happen in the active space; Super Node distillation always writes into the default space in Phase 1."
          >
            Shared Spaces
          </span>
          <button
          ...
          <div style={{ display: "grid", gap: isMobile ? 12 : 14, minWidth: 0 }}>
            <div ref={markdownBodyRef} style={{ minWidth: 0, fontSize: 13, lineHeight: 1.65 }}>
              ...
              {displaySourceRefs.length > 0 ? (
                <section aria-label="Super Node source refs" style={{ marginTop: 16, display: "grid", gap: 6 }}>
                  <Tiny style={{ fontWeight: 650 }}>Super Node source refs</Tiny>
    ...
    >
      ...
      {projectLink ? (
        <a href={projectLink} style={{ marginLeft: 8, color: "inherit", textDecoration: "underline" }}>
          Open Super Node for live state →
        </a>
      ) : null}
    </FreshnessChipShell>
  );
...
// ---------------------------------------------------------------------------
// History tab: native Super Node issue table for recent LLM Wiki operation
...
const MANAGED_SKILL_LABELS: Record<string, string> = {
  ...
  "wiki-lint": "Wiki Lint",
  "paperclip-distill": "Super Node Distill",
...
const SETTINGS_SECTIONS: ReadonlyArray<{
  ...
  { key: "spaces", label: "Spaces", description: "Destination spaces - folders, slugs, and folder health. Per-space Super Node indexing is not configurable yet." },
  { key: "distillation", label: "Distillation", description: "Super Node -> default space. Cursors, caps, and routines for the company-wide distillation pipeline." },
  ...
  { key: "events", label: "Ingestion Settings", description: "Super Node event capture into the default space (issues, comments, documents)." },
];
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 12 }}>
              ...
              <div>
                ...
                <Tiny style={{ marginTop: 6, fontSize: 13, color: tokens.fg, lineHeight: 1.55, maxWidth: 540 }}>
                  When enabled, the Wiki Maintainer reads Super Node issues, comments, and documents for this
                ...
                <Tiny style={{ marginTop: 6, fontSize: 13, color: tokens.fg, lineHeight: 1.55, maxWidth: 540 }}>
                  Other spaces do not receive Super Node-derived pages yet. They stay on manual and raw-file
                  ingest until per-space Super Node ingestion profiles ship.
                </Tiny>
              </div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <CardBody padding={14}>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
            <fieldset style={{ border: 0, padding: 0, margin: 0, display: "grid", gap: 6 }}>
              ...
              <Tiny>
                These filters narrow the Super Node source scope. The destination is always the default
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {activeSettingsSection === "root" ? (
              ...
              <div style={{ display: "grid", gap: 14, maxWidth: isMobile ? "none" : 620, minWidth: 0 }}>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  ...
                  {showMaintainerWarning ? (
                    <Callout tone="warn">
                      This is not the Super Node-provided Wiki Maintainer. Plugin operations and routines may miss the recommended wiki role, tools, and default instructions.
                    </Callout>
                  ) : null}
          ...
          <SettingsPanel
            ...
            description="Read Super Node issues, comments, and documents for this company and write project pages into the default wiki space. Assets/attachments and work products stay metadata-only in Phase 5 and are excluded from source-text extraction. Other spaces cannot be selected as a destination yet - that lands with per-space Super Node ingestion profiles."
          >
          ...
          <SettingsPanel
            title="Super Node event ingestion"
          ...
          <Tiny style={{ marginBottom: 10 }}>
            Company-scoped Super Node events can advance default-space cursors. Enable only the first-party text sources this wiki should observe for default-space distillation.
          </Tiny>
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <Callout tone="warn">
              Event ingestion records selected Super Node issue, comment, and document activity for the default wiki space. Assets/attachments and work products are excluded here: Phase 5 allows metadata-only references later, not blob reads or linked-content fetches. It never reads across companies or creates non-default space cursors.
            </Callout>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <CardBody>
          <div aria-disabled="true" style={{ display: "grid", gap: 10 }}>
            ...
            <Tiny>
              ...
              read/write permissions, and they do not change which Super Node sources reach this space.
            </Tiny>
...
function PaperclipIngestionSpaceCard({ companyId, space, refresh }: { companyId: string | null; space: WikiSpace; refresh: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      toast({ tone: "success", title: "Super Node ingestion profile saved", body: `${space.displayName} will use the selected Super Node sources.` });
  ...
  return (
    <Card>
      <CardHeader
        title={<span>Super Node → {space.displayName}</span>}
        right={<Badge tone={badge.tone} style={{ fontSize: 10 }}>{badge.label}</Badge>}
      />
      <CardBody>
        <div style={{ display: "grid", gap: 12 }}>
          {profileQuery.loading && !data ? <Tiny>Loading Super Node ingestion profile…</Tiny> : null}
          {profileQuery.error ? <Callout tone="danger">{profileQuery.error.message}</Callout> : null}
          {locked ? (
            <Callout tone="warn">
              Locked — host permissions pending. Super Node ingestion stays disabled on team and personal spaces until LLM Wiki enforces read/write permissions for non-shared spaces.
            </Callout>
          ) : null}
          {data && data.historicalPageCount > 0 && data.effectiveState === "disabled" ? (
            <Callout>
              Off · {data.historicalPageCount} historical Super Node page{data.historicalPageCount === 1 ? "" : "s"} still in this space. Disabling stops new observations but does not delete prior wiki pages.
            </Callout>
          ) : null}
          ...
          {draft ? (
            <>
              <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 13 }}>
                ...
                />
                <span>
                  Enable Super Node ingestion for this destination space
                  <Tiny style={{ display: "block" }}>Future Super Node issue, comment, and document events can advance cursors in {space.displayName}. Existing pages are preserved when this is turned off.</Tiny>
                </span>
              </label>
              <div style={{ display: "grid", gap: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <Button size="sm" variant="primary" onClick={save} loading={busy} disabled={!canSave || emptyScopes}>Save Super Node profile</Button>
