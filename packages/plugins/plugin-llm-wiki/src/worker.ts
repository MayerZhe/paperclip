...
function buildManualDistillPrompt(input: { companyId: string; projectId?: string | null; rootIssueId?: string | null }) {
  ...
  return [
    ...
    !input.projectId && !input.rootIssueId
      ? "- Do not hardcode a single project. Find non-plugin Super Node issues/comments/documents that changed in any project after the last processed cursor and are old enough for the stale/debounce threshold."
      : null,
    "",
    "Process:",
    "1. Read the wiki root AGENTS.md, wiki/index.md, and recent wiki/log.md entries.",
    "2. Assemble bounded Super Node source bundles for every eligible project or root issue, excluding LLM Wiki plugin-operation issues.",
  ...
  async setup(ctx) {
    ...
    for (const eventName of PAPERCLIP_EVENT_INGESTION_EVENTS) {
      ctx.events.on(eventName, async (event) => {
        ...
        if (result.status === "recorded") {
          ctx.logger.info("LLM Wiki recorded Super Node event for cursor discovery", {
    ...
    ctx.actions.register("queue-paperclip-ingestion-backfill", async (params) => {
      ...
      for (const scope of scopes) {
        ...
        const operation = await createOperationIssue(ctx, {
          ...
          spaceSlug,
          operationType: "backfill",
          title: scope.rootIssueId ? "Backfill Super Node root issue wiki history" : "Backfill Super Node project wiki history",
          useCheapModelProfile: params.useCheapModelProfile === true,
          prompt: [
            "Backfill LLM Wiki distillation was queued from a per-space Super Node ingestion profile.",
    ...
    ctx.actions.register("distill-paperclip-now", async (params) => {
      ...
      const operation = await createOperationIssue(ctx, {
        ...
        operationType: "distill",
        title: rootIssueId
          ? "Distill Super Node root issue into wiki"
          : projectId
            ? "Distill Super Node project into wiki"
            : "Distill Super Node changes into wiki",
        useCheapModelProfile: params.useCheapModelProfile === true,
    ...
    ctx.actions.register("backfill-paperclip-distillation", async (params) => {
      ...
      const operation = await createOperationIssue(ctx, {
        ...
        spaceSlug,
        operationType: "backfill",
        title: rootIssueId ? "Backfill Super Node root issue wiki history" : "Backfill Super Node project wiki history",
        useCheapModelProfile: params.useCheapModelProfile === true,
        prompt: [
          "Backfill LLM Wiki distillation requested for a bounded Super Node source window.",
