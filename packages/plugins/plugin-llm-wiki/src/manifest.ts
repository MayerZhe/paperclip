...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded Super Node issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). Super Node-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space Super Node ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up Super Node issue with the exact files and evidence.
...
const manifest: PaperclipPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, Super Node project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    },
    {
      ...
      displayName: "Super Node Distill",
      slug: PAPERCLIP_DISTILL_SKILL_KEY,
      description: "Turn Super Node cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
