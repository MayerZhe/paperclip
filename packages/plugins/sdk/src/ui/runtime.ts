...
type GlobalBridge = typeof globalThis & {
  __superNodePluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__superNodePluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `Super Node plugin UI runtime is not initialized for "${name}". ` +
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`Super Node plugin UI component "${name}" is not callable`);
}
