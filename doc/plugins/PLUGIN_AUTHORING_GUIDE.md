...
This guide describes the current, implemented way to create a Super Node plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Super Node app.
...
- The host provides a small shared React component kit through
  `@paperclipai/plugin-sdk/ui`; use it for common Super Node controls before
...
Outside this monorepo, the scaffold snapshots `@paperclipai/plugin-sdk` from the local Super Node checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/paperclip/packages/plugins/sdk` if you have more than one Super Node checkout.
...
## Managed Super Node resources

Plugins that provide durable Super Node business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Super Node agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Super Node issues. Prefer managed routines over
...
- Managed skills: declare top-level `skills[]` and require `skills.managed`.
  ...
  synced into Super Node managed agents.
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Super Node issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Super Node records that the
...
Super Node-native control. The host owns the implementation, so plugins inherit
the board's current styling, ordering, recent selections, and dark-mode behavior
without importing `ui/src` internals.

Prefer shared components for common Super Node UX patterns to reduce drift and
