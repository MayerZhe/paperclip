...
Super Node V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Super Node-shaped design.

Assumption for this document: Super Node is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Super Node objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Super Node.

The main conclusion is:

- Super Node should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Super Node should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Super Node should use multiple extension classes instead of one generic plugin bag:
...
If Super Node does this well, the examples you listed become straightforward:
...
Relevant Super Node files reviewed for current extension seams:
...
It is too dangerous for Super Node core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Super Node — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Super Node should absolutely do this.
...
Super Node should copy this discipline.
...
Super Node should adopt this style for plugin actions, automations, and UI schemas.
...
Super Node can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Super Node too.

## What Super Node Should Not Copy Directly
...
Super Node is a control plane for an operator-managed instance with company objects.
...
Super Node is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Super Node should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
For Super Node, equivalent hooks into:
...
Super Node should not allow plugins to silently replace:
...
For Super Node it would be risky because it combines:
...
Super Node should require an explicit operator install step.

## Why Super Node Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Super Node |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Super Node instance |
...
That means Super Node should borrow the good ideas from `opencode` but use a stricter architecture.

## Super Node Already Has Useful Pre-Plugin Seams

Super Node has several extension-like seams already:
...
This is good news.
Super Node does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Super Node Plugin Model
...
Super Node already has this pattern implicitly:
...
- `plugin` means integration code that talks to Super Node through a typed plugin protocol and capability model
...
- invoke explicit Super Node actions through the API
...
Super Node should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Super Node objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Super Node already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Super Node too.
...
A `@paperclipai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Super Node instance.

A `create-paperclip-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Super Node achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Super Node server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Super Node's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For Super Node
...
Any Super Node plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Super Node objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of Super Node itself
...
If a concept is becoming part of Super Node's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Super Node itself
...
### 3. Scope plugin data to Super Node objects before adding custom schemas

A lot of plugin data naturally hangs off existing Super Node objects:
...
If Super Node eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Super Node.
- Board reviews the diff after a run without leaving Super Node.
...
This plugin syncs Super Node work with Linear. It is useful for:

- importing backlog from Linear
- linking Super Node issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Super Node and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Super Node.
- Super Node issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs Super Node issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Super Node issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Super Node task governance
...
- Plugin settings:
  ...
  - whether new Super Node issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Super Node issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Super Node.
- GitHub webhooks update status/comment state in Super Node.
- A PR is linked back to the Super Node issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside Super Node. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Super Node issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Super Node plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Super Node dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Super Node issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Super Node core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into Super Node. It is useful for:
...
- Failed payment spikes or churn events can generate Super Node issues for follow-up.
...
- Stripe data should stay additive to Super Node core
...
Super Node should not implement "an OpenCode-style generic in-process hook system."
Super Node should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Super Node
