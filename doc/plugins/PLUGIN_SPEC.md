# Super Node Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Super Node's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Super Node app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Super Node HTTP APIs directly.
...
Super Node plugin design is based on the following assumptions:

1. Super Node is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Super Node business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Super Node core.
...
2. Let plugins add major capabilities without editing Super Node core.
...
The single Super Node deployment an operator installs and controls.

### 5.2 Company

A first-class Super Node business object inside the instance.
...
A trusted in-process extension loaded directly by Super Node core.
...
An installable instance-wide extension package loaded through the Super Node plugin runtime.
...
Super Node has two extension classes.
...
Super Node already has a concrete workspace model:
...
Plugins live under the Super Node instance directory.
...
Super Node should add CLI commands:
...
Plugins may contribute tools that Super Node agents can use during runs.
...
- Super Node server starts one worker process per installed plugin
...
Receives one typed Super Node domain event.
...
Trusted orchestration plugins can create and update Super Node issues through `ctx.issues` instead of importing server internals. The public issue contract includes parent/project/goal links, board or agent assignees, blocker IDs, labels, billing code, request depth, execution workspace inheritance, and plugin origin metadata.

Plugins that perform durable work should declare managed Super Node resources rather than using private plugin state:
...
6. For recurring business workflows that should create visible Super Node work, prefer managed routines and managed resources over jobs. Jobs remain useful for private plugin-runtime maintenance tasks.
...
Super Node-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Super Node navigation because those can force a full document reload.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Super Node secret provider system rather than a plain text input
...
1. Core Super Node data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Super Node objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Super Node product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Super Node later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Super Node secret provider system.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Super Node server. This is a normative requirement, not optional.
...
For developing a plugin against a running Super Node instance:
...
Super Node should not implement a generic in-process hook bag modeled directly after local coding tools.

Super Node should implement:
...
That is the complete target design for the Super Node plugin system.
