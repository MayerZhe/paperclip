...
1. Start Super Node in auth mode.
```bash
...
3. In Super Node UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Super Node UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Super Node invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Super Node with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Super Node UI that new issue exists.
