# LLM Wiki Super Node Asset And Work-Product Security Gate

Status: accepted Phase 5 policy
Date: 2026-05-06
Owner: Security engineering
Scope: Super Node-derived ingestion into the LLM Wiki before any asset or work-product content indexing ships

## Decision

Phase 5 remains **fail-closed** for Super Node assets and work products.

- Super Node-derived **text extraction is allowed only** for issue titles/descriptions, issue comments, and issue documents.
- Super Node **assets/attachments** and **issue work products** are **metadata-only** in Phase 5.
...
These source kinds may contribute body text to Super Node-derived source bundles:

| Source kind | Allowed body fields | Reason |
| --- | --- | --- |
| Issue | `title`, `description`, identifier/status metadata | First-party Super Node text under company ACL |
| Comment | `body` | First-party Super Node text under company ACL |
| Document | `body`, `title`, `key`, revision metadata | First-party Super Node text under company ACL |
...
- metadata-only references built from allowlisted structured fields already stored in Super Node
...
- Every asset MIME type is treated as opaque for Super Node-derived indexing.
...
- non-default-space routing for Super Node-derived asset/work-product references

## Security Rationale

This gate exists because the current host surfaces have different trust properties:

- issue/comment/document text is first-party Super Node content already exposed through company-scoped issue/document APIs
