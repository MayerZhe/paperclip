# Super Node Memory Service Plan

## Goal

Define a Super Node memory service and surface API that can sit above multiple memory backends, while preserving Super Node's control-plane requirements:

- company scoping
- auditability
- provenance back to Super Node work objects
- budget and cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md`, the AWS AgentCore comparison captured in [PAP-1274](/PAP/issues/PAP-1274), and the current Super Node architecture in:
...
Super Node should add a company-scoped memory control plane with company default plus agent override resolution, shared hook delivery, and full operation attribution, while leaving extraction and storage semantics to built-ins and plugins.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing Super Node plugin runtime.

That keeps the core small and matches the broader Super Node direction that specialized knowledge systems live at the edges.

### 4. Super Node owns routing, provenance, and policy

Providers should not decide how Super Node entities map to governance.

Super Node core should own:
...
### 5. Super Node exposes shared hooks, providers own extraction

Super Node should emit a common set of memory hooks that built-ins, third-party adapters, and plugins can all use.

Those hooks should pass structured Super Node source objects plus normalized metadata. The provider then decides how to extract from those objects.

Super Node should not force one extraction pipeline or one canonical "memory text" transform before the provider sees the input.
...
Super Node should still log those changes in activity and preserve full auditability. Approval-gated memory governance can remain an enterprise or future policy layer.
...
A mapping from a Super Node target to a binding.
...
The normalized Super Node scope passed into a provider request.
...
A normalized trigger emitted by Super Node when something memory-relevant happens.
...
A normalized capture, record-write, query, browse, get, correction, or delete action performed through Super Node.

Super Node should log every memory operation whether the provider is local, plugin-backed, or external.

## Required Adapter Contract

The required core should be small enough to fit `memsearch`, `mem0`, `Memori`, `MemOS`, or `OpenViking`, but strong enough to satisfy Super Node's attribution and inspectability requirements.
...
This contract intentionally does not force a provider to expose its internal graph, file tree, or ontology. It does require enough structure for Super Node to browse, attribute, and audit what happened.
...
That lines up with the Super Node plan at a high level: provider configuration, scoped writes, scoped retrieval, provider-managed extraction as a capability, and a browse and inspect surface.

The concrete changes Super Node should take from AWS are:
...
AWS exposes list and retrieve surfaces directly. Super Node should not make browse optional at the portable layer.
...
Super Node should add cursor-based pagination to:
...
Super Node should keep its own control-plane-centric model, but the adapter contract needs obvious places to map those concepts:
...
AWS exposes extraction jobs explicitly. Super Node should too.
...
### 7. Keep Super Node provenance primary

Super Node should continue to center:
...
## What Super Node Should Persist

Super Node should not mirror the full provider memory corpus into Postgres unless the provider is a Super Node-managed local provider.

Super Node core should persist:
...
Super Node should expose one shared hook system for memory.
...
1. `pre_run_hydrate`
   Before an agent run starts, Super Node may call `query(... intent = "agent_preamble")` using the active binding.

2. `post_run_capture`
   After a run finishes, Super Node may call `capture(...)` with structured run output, excerpts, and provenance.

3. `issue_comment_capture`
   When enabled on the binding, Super Node may call `capture(...)` for selected issue comments.

4. `issue_document_capture`
   When enabled on the binding, Super Node may call `capture(...)` for selected issue documents.
...
Super Node should give agents both automatic recall and explicit tools, with simple guidance:
...
Super Node needs a first-class UI for memory, otherwise providers become black boxes.
...
Super Node should treat memory accounting as two related but distinct concerns:
...
- if a memory operation runs inside a normal Super Node agent heartbeat and the model usage is already counted on that run, do not create a duplicate `cost_event`
- instead, store the memory operation with `attributionMode = "included_in_run"` and link it to the related `heartbeatRunId`
- if a memory provider makes a direct metered model call outside the agent run accounting path, the provider must report usage and Super Node should create a `cost_event`
...
Super Node should record evaluation-oriented metrics where possible:
...
This is important because a memory system that "works" but silently burns budget or silently fails extraction is not acceptable in Super Node.
...
- it matches Super Node's local-first posture
- it is inspectable
...
- How much source payload should Super Node snapshot inside `memory_operations` for debugging without duplicating large transcripts?
...
- Super Node owns bindings, resolution, hooks, provenance, policy, and attribution.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives Super Node a stable memory service without locking the product to one memory philosophy or one vendor, and it integrates the AWS lessons without importing AWS's model into core.
