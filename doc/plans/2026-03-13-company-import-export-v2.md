...
This document defines the next-stage plan for Super Node company import/export.

The core shift is:

- move from a Super Node-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Super Node.
...
Super Node already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Super Node
...
7. an always-emitted `.paperclip.yaml` sidecar for high-fidelity Super Node-specific details
...
11. adapter-aware skill sync surfaces so Super Node can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Super Node at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Super Node sidecar.
...
Super Node must not redefine `SKILL.md`.
...
- Super Node-specific fidelity lives in `.paperclip.yaml`
- Super Node may resolve and install `SKILL.md` packages, but it must not require a Super Node-only skill format
...
### 5.4 Base Package Vs Super Node Extension
...
- base package:
  ...
  - no Super Node-only runtime fields by default
- Super Node extension:
  - `.paperclip.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Super Node tools as a sidecar while the base package stays readable
...
Super Node import/export should support these entity kinds:
...
- Super Node can treat “this set of agents came from team package X” as the imported-team model
...
Super Node should support source references in package metadata with:
...
Super Node should define a new adapter surface area around skills:
...
Planned Super Node behavior:

- if an adapter supports read, Super Node should show current skills in the UI
- if an adapter supports write, Super Node should let the user enable/disable imported skills
- if an adapter supports sync, Super Node should compute desired vs actual state and offer reconcile actions
...
## 10. Storage Model Inside Super Node
...
Super Node should add managed package/provenance records so imports are not anonymous one-off copies.
...
1. Should imported skill packages be stored as managed package files in Super Node storage, or only referenced at import time?
...
3. Should Super Node support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Super Node aligned with:
