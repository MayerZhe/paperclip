...
Turn the `agent-os` research into a low-risk Super Node execution plan that preserves Super Node's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Super Node should not absorb `agent-os` as a product model or orchestration layer.

Super Node should evaluate `agent-os` in three narrow areas:
...
This work is successful when Super Node has:
...
- a proposed runtime capability model that fits current Super Node adapters
...
- replace Super Node heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Super Node core

## Existing Super Node integration points
...
- prove that Super Node can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Super Node's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Super Node-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Super Node workloads
...
- identify where Super Node should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Super Node
...
- runtime experimentation could blur the boundary between Super Node as control plane and Super Node as execution platform
