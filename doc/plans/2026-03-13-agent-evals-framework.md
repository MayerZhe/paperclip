...
We need evals for the thing Super Node actually ships:
...
- can we grow the suite over time from real Super Node usage?
...
Super Node should take a **two-stage approach**:

1. **Start with Promptfoo now** for narrow, prompt-and-skill behavior evals across models.
2. **Grow toward a first-party, repo-local eval harness in TypeScript** for full Super Node scenario evals.
...
3. The longer-term harness should run **real Super Node scenarios** against seeded companies/issues/agents, not just raw prompt completions.
...
That is the right unit because that is what actually changes behavior in Super Node.

## Why This Is The Right Shape

### 1. We need to evaluate system behavior, not only prompt output

Prompt-only tools are useful, but Super Node’s real failure modes are often:
...
But OpenAI’s Evals API is not the right control plane for Super Node as the primary system because our target is explicitly multi-model and multi-provider.
...
But Super Node should still avoid making a hosted platform or a third-party config format the core abstraction before we have our own stable eval model.

The right move is:

- start with Promptfoo for quick wins
- keep the data portable and repo-owned
- build a thin first-party harness around Super Node concepts as the system grows
...
These are the closest thing to prompt evals, but still framed in Super Node terms.
...
The important part is that the case is about a Super Node scenario, not a standalone prompt string.
...
Super Node should grow eval coverage by promoting real runs into cases when we see:
...
- Super Node needs scenario setup, control-plane state inspection, and multi-step traces as first-class concepts

Recommendation:

- use Promptfoo first
- store Promptfoo config and cases in-repo under `evals/promptfoo/`
- use custom JS/TS assertions and, if needed later, a custom provider that calls Super Node scenario runners
- do not make Promptfoo YAML the only canonical Super Node eval format once we outgrow prompt-level evals
...
- Super Node must compare across models/providers
- we do not want our primary eval runner coupled to one model vendor

Recommendation:

- use the guidance
- do not use it as the core Super Node eval runtime
...
- did they call the right Super Node APIs?
...
Start with Promptfoo for immediate, narrow model-and-prompt comparisons, then grow into a first-party `evals/` framework in TypeScript that evaluates **Super Node scenarios and bundles**, not just prompts.
...
But keep the canonical eval model inside the Super Node repo and aligned to Super Node’s actual control-plane behaviors.
