...
Super Node already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Super Node should implement next.

## Product Goals

Super Node should let operators:
...
For the next implementation pass, Super Node should enforce these defaults:
...
Super Node should separate subscription-included usage from billed spend:
...
Super Node should have two threshold classes:
...
Super Node must know that a project is budget-paused and enforce it.
...
Super Node must also block execution before new work starts.
