...
Super Node should not replace its workspace runtime service model with VS Code tasks.
...
- Super Node owns long-running workspace services and their desired state
- Super Node shows operators exactly which named thing they are starting or stopping
- Super Node distinguishes long-running services from one-shot jobs
...
This is directionally correct for Super Node because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Super Node does not yet leverage existing workspace metadata.

## Recommendation

Keep Super Node runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Super Node.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Super Node service or job semantics should become runnable in Super Node.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Super Node runtime services.
...
That creates a bad fit if Super Node tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Super Node's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Super Node should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Super Node can resolve
...
- selected variable substitution when Super Node can resolve it safely from workspace context
- mapping task metadata into Super Node readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Super Node cannot execute directly
...
Super Node needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Super Node metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Super Node
...
For a workspace with `cwd`, Super Node should look for `.vscode/tasks.json`.
...
Super Node should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Super Node metadata marks it as a service, or
...
Super Node-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Super Node would import and why.
...
Goal:
run supported imported service commands through the current Super Node supervisor.
...
Success condition:
imported service commands behave like native Super Node services once adopted.
...
- removal of Super Node's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Super Node to VS Code and making the model worse for non-VS-Code repos
...
Super Node should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Super Node workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
