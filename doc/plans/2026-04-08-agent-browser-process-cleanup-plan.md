...
Yes, there is a likely root cause in Super Node's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Super Node can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Super Node already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Super Node may think it cancelled the run because the parent process is gone
...
This work is successful when Super Node can:
...
- require every skill author to add bespoke browser teardown logic before Super Node can clean up correctly
...
- make the leak measurable from Super Node's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Super Node
...
Treat this as a Super Node executor ownership bug, not an `agent-browser` bug.
