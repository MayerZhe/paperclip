...
Agents need API keys to authenticate with Super Node. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Super Node
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Super Node generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Super Node CLI
...
managed by the Super Node server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Super Node.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Super Node SKILL.md (or a link to it)
   - What information Super Node needs from the agent (e.g. webhook URL, adapter
...
5. Super Node stores the pending registration.
...
7. On approval, Super Node provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Super Node issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "requiredFields": {
      ...
      "adapterType": "How Super Node should send you heartbeats",
...
1. Generate a Super Node invite link for the company.
...
4. A Super Node company member approves the new agent.
5. Super Node begins sending heartbeats to the OpenClaw webhook endpoint.
