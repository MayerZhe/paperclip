...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Super Node plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Super Node.
...
- subscribe to a core Super Node event such as `issue.created`
...
Recommendation: default to a Super Node-local endpoint or a stable public echo endpoint to avoid flaky docs.
