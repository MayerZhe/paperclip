...
This document defines a V1 plan for "smart model routing" in Super Node.
...
- preserve Super Node's existing task, session, and audit invariants
...
That separation is a better fit for Super Node than copying Hermes' exact keyword heuristic.

## 3. Current Super Node State

Super Node already has the right execution shape for adapter-specific routing, but it currently assumes one model per heartbeat run.
...
Super Node should implement smart model routing as an adapter-local, opt-in execution pattern.

V1 decision:

1. Do not add a global server-side router that tries to understand every adapter.
2. Do not copy Hermes' prompt-keyword classifier as Super Node's default routing policy.
...
- Super Node heartbeats are structured, issue-scoped, and already include wake metadata
...
- the adapter already injects Super Node skills and workspace metadata cleanly
...
This matters because Super Node's board UI is supposed to make cost and behavior legible.

## 9. Why Not Copy Hermes Exactly

Hermes' cheap-route heuristic is useful precedent, but Super Node should not start there.

Reasons:

- Hermes is optimizing free-form conversational turns
- Super Node agents run structured, issue-scoped heartbeats with explicit task and workspace context
- Super Node already knows whether a run is fresh vs resumed, issue-scoped vs approval follow-up, and what workspace/session exists
- those execution facts are stronger routing signals than prompt keyword matching

If Super Node later wants a cheap-only completion path for trivial runs, that can be a second-stage feature built on observed run data, not the first implementation.
...
Super Node should ship smart model routing as:
