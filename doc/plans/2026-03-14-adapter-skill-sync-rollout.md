...
This document defines the rollout plan for adapter-wide skill support in Super Node.
...
- Super Node stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Super Node currently has these adapters:
...
1. Desired skills live in Super Node for every adapter.
...
These adapters have a stable local skills directory that Super Node can read and manage.
...
These adapters do not have a meaningful Super Node-owned persistent install state.
...
- show desired Super Node skills
...
- desired skills are stored in Super Node
...
- desired skills stored in Super Node
...
- Super Node reconciles desired skills into `~/.cursor/skills`
...
- Super Node reconciles desired skills into `~/.gemini/skills`
...
- Super Node reconciles desired skills into `~/.pi/agent/skills`
...
- Super Node can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Super Node skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Super Node may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Super Node skills that are clearly marked as Super Node-managed
...
- Super Node stores desired skills only
...
- avoid destructive wording unless Super Node can prove a skill is Super Node-managed
