...
`PAP-447` asks how Super Node should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Super Node instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Super Node:

- some operators use Super Node to manage Super Node and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Super Node
- Claude and Codex expose different built-in affordances, so Super Node should not overfit to one tool

## Core Product Decision

Super Node should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Super Node
...
- Super Node should not assume that every adapter can see or use a host filesystem path directly
...
If Super Node makes worktrees agent-first, it will blur:
...
- Super Node core resolves and records execution workspace state
...
That gives Super Node room to support:
...
- Super Node handles workspace runtime services under the hood using project/workspace policy
...
Once Super Node is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Super Node should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Super Node cannot infer
...
- when issue moves to done, should Super Node auto-commit?
- should it auto-push?
...
Super Node already has the right foundation for a project-first model.
...
For server-spawned local adapters, Super Node already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Super Node should mirror directly

Implication:

- for `codex_local`, Super Node should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Super Node should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Super Node should separate:
...
Super Node should be able to express intentions such as:
...
These are not specific to Super Node. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a Super Node-specific `devServer` flag.
...
Super Node should distinguish between:
...
Super Node should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Super Node should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Super Node
...
For runtime services specifically, Super Node should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Super Node-only dev-server flags
...
- same embedded DB / Super Node instance can be reused through env/config injection when appropriate
...
- Super Node can decide whether to reuse or start a fresh service deterministically
...
3. Test against Super Node-on-Super Node workflows first.
...
7. Workspace runtime services are modeled generically, not as Super Node-specific dev-server toggles.
