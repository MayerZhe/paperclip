...
This document defines the product and UI plan for skill management in Super Node.
...
- Super Node company import/export can include skills as package content
...
3. The UI must always tell the truth about what Super Node knows:
   - desired state in Super Node
...
Super Node should treat skills at two scopes:
...
- created locally inside Super Node later
...
7. “Understand what Super Node knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Super Node-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Super Node did not install.

These should be clearly marked:

- external
- not managed by Super Node
...
- `desired_only`: Super Node wants it, adapter does not show it yet
- `external`: adapter has it but Super Node does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Super Node only tracks desired state
...
- Super Node can still track desired skills
...
- Super Node can see adapter skills
...
- `.paperclip.yaml` may add Super Node-specific fidelity, but should not replace the base shortname association model
...
- `.paperclip.yaml` may add Super Node-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Super Node should have a company-scoped skill model or managed package model representing:
...
Super Node should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Super Node-specific.
...
That gives Super Node one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
