...
Date: 2026-05-26
Source plan: approved Super Node skills CLI and catalog plan
...
- `paperclipai skills` manages Super Node company skills. It does not manage
...
- Root `skills/` remains reserved for Super Node runtime and operational skills.
...
| `skills agent sync <agent-ref> --skill <skill-ref>...` | Replaces the agent's non-required desired skill set with the supplied refs and triggers adapter sync. Required Super Node skills remain enforced by the server. | `AgentSkillSnapshot` |
...
Catalog commands are for the app-shipped Super Node catalog only. External GitHub,
