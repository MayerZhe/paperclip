...
Super Node currently stores model spend in `cost_events` and operational run state in `heartbeat_runs`.
...
However, if Super Node needs to account for the full billing surface of aggregators and managed AI platforms, then `cost_events` alone is not enough.
...
It follows directly from the billing systems of providers and aggregators Super Node needs to support.
