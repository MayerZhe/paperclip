...
Super Node's current release flow is documented in `doc/RELEASING.md` and implemented through:
...
Move Super Node to semver-compatible calendar versioning, auto-publish canaries from `master`, promote stable from a chosen tested commit, and use npm trusted publishing plus GitHub environments so no long-lived npm or LLM token needs to live in Actions.

## Core Decisions

### 1. Use calendar versions, but keep semver syntax

The repo and npm tooling still assume semver-shaped version strings in many places. That does not mean Super Node must keep semver as a product policy. It does mean the version format should remain semver-valid.
...
That tradeoff is probably acceptable for Super Node, but it should be explicit:
...
Super Node's release problem is now "publish the whole fixed package set at one explicit version", not "derive the next semantic bump from human intent".
...
Super Node should adopt this model:
