...
- local runtime data from the default Super Node instance on 2026-03-13
...
- Super Node discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Super Node skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Super Node-side skill changes land.
...
- Store both:
  ...
  - Super Node-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Super Node to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
- For `codex_local`, either:
  ...
  - treat Super Node-owned Codex skill symlinks as repairable when they point at a different checkout
