...
This document defines the V1 design for a Super Node adapter that integrates with
...
4. synthesized stdout events for Super Node UI/CLI
...
3. **Agent callback auth to Super Node** uses a bootstrap exchange flow (no long-lived Super Node key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Super Node endpoints, not full SKILL.md prompt injection.
...
## Super Node Callback + Auth Flow (V1)
...
Goal: avoid putting long-lived Super Node credentials in prompt text.

Flow:

1. Before launch/follow-up, Super Node mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Super Node validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Super Node API calls.
...
1. Prompt includes a compact instruction to fetch skills from Super Node.
...
4. Route by Cursor `agentId` to active Super Node run context.
...
- poll interval
- timeout/grace
- Super Node public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from Super Node skill endpoints |
| Super Node auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Super Node context.
3. Add richer PR metadata surfacing in Super Node UI.
