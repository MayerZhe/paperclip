# Agent OS Technical Report for Super Node
...
`agent-os` is not a competitor to Super Node's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. Super Node is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that Super Node could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in Super Node.

My recommendation is:

1. Do not merge agent-os concepts into the Super Node core product model.
...
4. Do not import its workflow/cron/queue abstractions into Super Node core until they are reconciled with Super Node's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than Super Node's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to Super Node because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## Super Node anchor points

The most relevant current Super Node surfaces for any future `agent-os` integration are:
...
## What Super Node can learn from it

### 1. A safer local execution substrate

Super Node's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits Super Node because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

Super Node already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
Super Node has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that Super Node could adopt even without adopting the runtime itself:
...
Super Node's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.

Concrete Super Node uses:
...
This aligns well with Super Node's governance story.

### 5. Better adapter normalization around sessions and capabilities

Super Node's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
Super Node does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
Super Node can adopt that philosophy directly:
...
## What does not fit Super Node well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. Super Node already has higher-level orchestration concepts:
...
If Super Node copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

Super Node should keep orchestration authoritative at the control-plane layer.
...
Those are Super Node's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. Super Node would still need to answer:
...
- real integration work for Codex/Cursor/Gemini-style Super Node adapters

So the main near-term value is not universal replacement. It is selective use where compatibility is strong.

## Concrete recommendations for Super Node
...
- keep Super Node heartbeat/task/workspace/budget logic unchanged
...
- Super Node's Codex adapter is already important and carries repo-specific behavior
...
- Super Node still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, Super Node should consider standardizing adapter/runtime permissions around a vocabulary like:
...
Super Node should evaluate whether some execution workspaces can be backed by:
...
Super Node plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into Super Node core

Super Node should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving Super Node orchestration into agent-os workflows
...
`agent-os` is useful to Super Node as an execution technology reference, not as a product model.

Super Node should treat it the same way it treats sandboxes or agent CLIs:
