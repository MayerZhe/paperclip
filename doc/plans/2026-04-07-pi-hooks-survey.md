...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Super Node.
...
- current Super Node plugin and adapter surfaces in this repo
...
- Super Node should copy only the safe subset:
  - typed event subscriptions
  - read-only run lifecycle events
  - explicit worker lifecycle hooks
  - plugin-to-plugin events
- Super Node should not copy the dangerous subset:
...
## What Super Node already has

Super Node already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Super Node

These ideas from `pi-mono` fit Super Node with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Super Node should continue exposing run and transcript events to plugins, for example:
...
This matches Super Node's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Super Node already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Super Node already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Super Node, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Super Node core:

### 1. Arbitrary mutation hooks on control-plane decisions

Super Node should not let general plugins rewrite:
...
Super Node should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Super Node is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Super Node's board-and-issues model.

## Recommended Super Node direction
...
If the question is "what works for Super Node too?":
