...
Super Node needs to support two very different but equally valid ways of working:
...
Today, Super Node already has the beginnings of this model:
...
- making Super Node feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Super Node users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Super Node deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Super Node into a full code review product
...
This object must be recorded explicitly so that Super Node can:
...
Super Node should treat PRs as a type of work product linked back to:
...
Super Node already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Super Node must be a first-class deployment mode

Super Node cannot assume that it is running on the same machine as the code.

In cloud deployments, Super Node may:
...
- `Runtime service`: a process or service Super Node owns or tracks for a workspace
...
It also lets cloud-hosted Super Node deployments point at codebases and remotes without pretending the Super Node host has direct filesystem access.
...
Without an explicit `execution workspace` record, Super Node has nowhere stable to attach:
...
Super Node needs a single place to show:
...
- Super Node-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Super Node owning the execution host
...
## 1. Super Node host and execution host must be decoupled
...
without the Super Node host itself running local git or local preview processes.
...
Super Node should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Super Node.
...
Super Node should keep workspace automation centered on a small set of hooks:
...
Super Node should provide the equivalent visibility in the project `Code` surface:
...
Super Node should treat execution workspaces as the natural home for:
...
Super Node should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Super Node should continue treating cleanup policy as part of the core workspace model:
...
Super Node should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Super Node host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Super Node should borrow Worktrunk's good execution patterns while keeping the broader Super Node model:
...
This is how Super Node supports:
...
That keeps Super Node focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Super Node deployments remain first-class

That is a better fit for Super Node than either extreme:
