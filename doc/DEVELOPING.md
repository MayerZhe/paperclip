...
`pnpm dev` and `pnpm dev:once` are now idempotent for the current repo and instance: if the matching Super Node dev runner is already alive, Super Node reports the existing process instead of starting a duplicate.
...
Build and run Super Node in Docker:
...
Use the helper bundled with the Super Node skill from the repo root:
...
When a local agent run has no resolved project/session workspace, Super Node falls back to an agent home workspace under the instance root:

- `~/.paperclip/instances/default/workspaces/<agent-id>`

This path honors `PAPERCLIP_HOME` and `PAPERCLIP_INSTANCE_ID` in non-default setups.

For `codex_local`, Super Node also manages a per-company Codex home under the instance root and seeds it from the shared Codex login/config home (`$CODEX_HOME` or `~/.codex`):

- `~/.paperclip/instances/default/companies/<company-id>/codex-home`

If the `codex` CLI is not installed or not on `PATH`, `codex_local` agent runs fail at execution time with a clear adapter error. Quota polling uses a short-lived `codex app-server` subprocess: when `codex` cannot be spawned, that provider reports `ok: false` in aggregated quota results and the API server keeps running (it must not exit on a missing binary).

Local adapters require their corresponding CLI/session setup on the machine running Super Node. External adapters are installed through the adapter/plugin flow and should not require hardcoded imports in `server/` or `ui/`.

## Worktree-local Instances

When developing from multiple git worktrees, do not point two Super Node servers at the same embedded PostgreSQL data directory.

Instead, create a repo-local Super Node config plus an isolated instance for the worktree:
...
- by default seeds the isolated DB in `minimal` mode from the current effective Super Node instance/config (repo-local worktree config when present, otherwise the default instance) via a logical SQL snapshot
...
Seeded worktree instances quarantine copied live execution by default for both `minimal` and `full` seeds. During restore, Super Node disables copied agent timer heartbeats, resets copied `running` agents to `idle`, blocks and unassigns copied agent-owned `in_progress` issues, and unassigns copied agent-owned `todo`/`in_review` issues. This keeps a freshly booted worktree from starting agents for work already owned by the source instance. Pass `--preserve-live-work` only when you intentionally want the isolated worktree to resume copied assignments.
...
For an already-created worktree where you want to keep the existing repo-local config/env and only overwrite the isolated database, use `worktree reseed` instead. Stop the target worktree's Super Node server first so the command can replace the DB safely.

**`pnpm paperclipai worktree reseed [options]`** — Re-seed an existing worktree-local instance from another Super Node instance or worktree while preserving the target worktree's current config, ports, and instance identity.
...
**`pnpm paperclipai worktree:make <name> [options]`** — Create `~/NAME` as a git worktree, then initialize an isolated Super Node instance inside it. This combines `git worktree add` with `worktree init` in a single step.
...
**`pnpm paperclipai worktree env [options]`** — Print shell exports for the current worktree-local Super Node instance.
...
For project execution worktrees, Super Node can also run a project-defined provision command after it creates or reuses an isolated git worktree. Configure this on the project's execution workspace policy (`workspaceStrategy.provisionCommand`). The command runs inside the derived worktree and receives `PAPERCLIP_WORKSPACE_*`, `PAPERCLIP_PROJECT_ID`, `PAPERCLIP_AGENT_ID`, and `PAPERCLIP_ISSUE_*` environment variables so each repo can bootstrap itself however it wants.

## App-Shipped Skills Catalog

The Super Node app ships a curated catalog of company skills out of the box. The
...
paths at request time. Root `skills/` remains reserved for Super Node runtime
...
Super Node can run automatic logical database backups on a timer. These backups cover
...
Super Node CLI now includes client-side control-plane commands in addition to setup commands.
...
- `GET /api/skills/paperclip` returns the Super Node heartbeat skill markdown.
...
- auto-detects and prints a Super Node host URL reachable from inside OpenClaw Docker
- default container-side host alias is `host.docker.internal` (override with `PAPERCLIP_HOST_FROM_CONTAINER` / `PAPERCLIP_HOST_PORT`)
- if Super Node rejects container hostnames in authenticated/private mode, allow `host.docker.internal` via `pnpm paperclipai allowed-hostname host.docker.internal` and restart Super Node
