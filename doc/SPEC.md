# Super Node Specification

Target specification for the Super Node control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One Super Node instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the Super Node protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

Super Node doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### Super Node Protocol (What Super Node Knows)

At the protocol level, Super Node tracks:
...
- **Fat payload** — Super Node bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to Super Node.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls Super Node's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a Super Node agent: **be callable.** That's it. Super Node can invoke you via command or webhook. No requirement to report back — Super Node infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Super Node provides progressively richer integration:

1. **Callable** (minimum) — Super Node can start you. That's the only contract.
...
Super Node ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **Super Node skill** (a Claude Code skill for interacting with the Super Node API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the Super Node SKILL.md which is recommended for all agents. Super Node the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. Super Node defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how Super Node invokes the agent. Built-in adapters include:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets Super Node check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the Super Node REST API.

### What Super Node Controls
...
### What Super Node Does NOT Control
...
- Health monitoring — how does Super Node distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of Super Node. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to Super Node. Costs are tracked at every level:
...
Super Node ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **Super Node Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### Super Node Skill (SKILL.md)

A skill definition that teaches agents how to interact with Super Node. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my Super Node instance."

#### Agent Authentication

When a user creates an Agent, Super Node generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. Super Node generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), Super Node sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the Super Node SKILL, but sometimes it's unavoidable.
...
Super Node manages task-linked work artifacts: issue documents (rich-text plans, specs, notes attached to issues) and file attachments. Agents read and write these through the API as part of normal task execution. Full delivery infrastructure (code repos, deployments, production runtime) remains the agent's domain — Super Node orchestrates the work, not the build pipeline.
...
When an agent crashes or disappears mid-task, Super Node does **not** auto-reassign or auto-release the task. Instead:

- Super Node surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Super Node does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: Super Node reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Super Node system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
**Full loop with one adapter.** V1 must demonstrate the complete Super Node cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with Super Node skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **Super Node skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the Super Node core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things Super Node explicitly does **not** do:

- **Not an Agent runtime** — Super Node orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. Super Node is the control plane, not the execution plane.
