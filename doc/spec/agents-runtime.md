...
For `claude_local` and `codex_local`, Super Node assumes the CLI is already installed and authenticated on the host machine.
...
Super Node stores resumable session state per `(agent, taskKey, adapterType)`.
