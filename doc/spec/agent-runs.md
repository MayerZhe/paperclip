...
This spec defines how Super Node actually runs agents while staying runtime-agnostic.
...
1. Super Node is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Super Node manages lifecycle, and on exit Super Node parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared Super Node skill text block)
