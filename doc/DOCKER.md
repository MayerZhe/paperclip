...
Run Super Node in Docker without installing Node or pnpm locally.
...
Super Node server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). Super Node data persists in `paperclip-data`.
...
For authenticated deployments, set one canonical public URL and let Super Node derive auth/callback defaults:
...
Super Node URL, sign in or create an account, then choose `Claim this instance`
...
- Adapter environment checks in Super Node will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run Super Node + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/paperclip.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/paperclip.container` | Super Node server — joins the pod, connects to Postgres at `127.0.0.1` |
...
- Containers in a pod share `localhost`, so Super Node reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `paperclip-pgdata` named volume.
- Super Node data persists at `~/.local/share/paperclip`.
...
- Smoke script defaults host port to `3131` to avoid conflicts with local Super Node on `3100`.
...
- Super Node data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/paperclip` (quadlet).
