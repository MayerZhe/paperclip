# Super Node V1 Implementation Spec
...
Super Node V1 must provide a full control-plane loop for autonomous agents:
...
- `runtime_config` jsonb not null default `{}`; may include Super Node runtime policy such as `modelProfiles.cheap.adapterConfig` for an optional low-cost model lane that does not change the primary adapter config
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Super Node runtime-owned keys are injected

Routine execution issues add a routine-scoped env overlay after project env and before Super Node runtime-owned keys. Routine env uses the same secret-aware binding format, is stored on `routines.env`, is snapshotted in routine revisions, and resolves secret refs against the routine binding target so routine-owned secrets do not require direct bindings on the executing agent.
...
- when Super Node cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
Super Node V1 keeps a company-scoped visibility model as the default because centralized authorization and scoped work-object controls are not yet a core V1 control surface.
...
- `.paperclip.yaml` sidecar for Super Node-specific fidelity
...
- recurring `TASK.md` entries use `recurring: true` in the base package and Super Node routine fidelity in `.paperclip.yaml`
- Super Node imports recurring task packages as routines instead of downgrading them to one-time issues
