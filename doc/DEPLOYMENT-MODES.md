...
Super Node supports two runtime modes:
...
Super Node now treats **bind** as a separate concern from auth:
...
When running `authenticated` mode, if the only instance admin is `local-board`, Super Node emits a startup warning with a one-time high-entropy claim URL.
...
For `authenticated/private`, Super Node supports a browser-first setup path:

1. open the Super Node URL from the private network or appliance UI
2. sign in or create a Super Node account
