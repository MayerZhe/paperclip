...
Operational contract for the hosted `aws_secrets_manager` secret provider used by Super Node Cloud.

## Scope

- Hosted provider for Super Node-managed secrets when Super Node Cloud runs on AWS.
- Source of truth for secret values is AWS Secrets Manager, not Postgres.
- Super Node stores only metadata needed for ownership, bindings, version selection, audit, and runtime resolution.
- AWS provider bootstrap credentials are deployment/runtime credentials, not Super Node-managed company secrets.
- Remote import for existing AWS secrets is metadata-only. Preview/import uses
  AWS inventory metadata and creates Super Node external references; it does not
  copy plaintext into Super Node.
...
The AWS provider has a chicken-and-egg boundary: Super Node cannot use
`company_secrets` to unlock the AWS provider that stores those secrets. The
initial AWS trust must exist before the Super Node server starts.

Allowed bootstrap locations:

- Infrastructure IAM or workload identity attached to the Super Node server
  runtime.
- Process environment or orchestrator secret store used to start the Super Node
  server.
...
keys into the Super Node board UI. Do not store those bootstrap keys in
`company_secrets`.

## Super Node Cloud Bootstrap

Super Node Cloud must provision the AWS backing resources before any board user
can create AWS-backed company secrets:

1. Create or select the deployment KMS key.
2. Create the Super Node server runtime role for the deployment.
...
Once this is in place, the board UI can create Super Node-managed AWS secrets and
Super Node will write them under the deployment/company namespace.
...
should not be written to Super Node config, committed to `.env` files, stored in
`company_secrets`, or used as the default Super Node Cloud bootstrap path.
...
Naming convention for Super Node-managed secrets:

```text
paperclip/{deploymentId}/{companyId}/{secretKey}
```

Tag set for Super Node-managed secrets:
...
- One Super Node app role per deployment.
- One deployment-scoped KMS key per deployment at launch.
- Future per-company KMS keys remain compatible because Super Node stores provider refs and version metadata separately from values.
...
- Prefer workload identity / role-based auth. Do not store AWS credentials inline in Super Node config.
...
- Super Node-managed secrets may be deleted only by Super Node or an operator with equivalent break-glass access.
- External references may resolve through Super Node runtime, but Super Node should not delete the external secret resource.
...
operator-approved external prefixes that Super Node is allowed to consume:
...
- Prefer one Super Node runtime role per environment/account.
...
Super Node also blocks importing refs under its own managed namespace as
external references. Use the Super Node-managed flow for
...
Super Node-managed resources.

Use the Super Node-managed flow when Super Node should create and rotate the value. The AWS
...
In that mode Super Node stores only the path or ARN, resolves it at runtime, and records
redacted access events. Operators rotate the actual value in AWS. Update the Super Node
reference only when the AWS path, ARN, or pinned provider version changes.

Super Node does not currently offer an "adopt existing AWS secret" flow that takes over future
`PutSecretValue` writes for an arbitrary existing secret. Adding that later requires explicit
confirmation UX, scope validation, expected Super Node tags, and security/cloud-ops review.

## Data Custody

- Super Node stores `externalRef`, `providerVersionRef`, provider id, fingerprint hash, status, and binding metadata.
- Super Node does not store AWS secret plaintext in `company_secret_versions.material`.
- Runtime resolution fetches the value from AWS only when a bound consumer needs it.

## Rotation Runbook

Manual Super Node-managed rotation:

1. Write the new value through the Super Node secret rotate flow.
2. Super Node creates a new AWS secret version with `PutSecretValue`.
3. Super Node records the new `providerVersionRef` in `company_secret_versions`.
4. Re-run or restart affected workloads that consume `latest`, or pin consumers to a specific Super Node version before rollout when you need staged release safety.

Guidance:

- Prefer pinned Super Node secret versions for risky rollouts.
...
- Super Node database metadata for secret ownership, bindings, status, and provider version refs.
- AWS Secrets Manager namespace under the configured deployment prefix.
- The configured KMS key and its decrypt permissions.

Restore checklist:

1. Restore Super Node database metadata.
2. Confirm the same AWS Secrets Manager namespace still exists.
3. Confirm the Super Node runtime role can call `GetSecretValue` on the restored prefix.
...
1. Stop or pause affected Super Node runs.
2. Audit recent Super Node secret access events for impacted secret ids and consumers.
...
4. Rotate impacted secrets in AWS through Super Node-managed versioning.
...
1. Create a test secret through the Super Node board or API under a throwaway company.
2. Confirm the resulting AWS secret name matches `paperclip/{deploymentId}/{companyId}/{secretKey}`.
3. Rotate the secret once and confirm a new `providerVersionRef` appears in Super Node metadata.
