...
Super Node CLI now supports both:
...
4. local Super Node config server port
...
Issue subresources are exposed as Super Node API wrappers. Commands that map to broad server schemas accept JSON payloads and validate them with shared schemas before sending.
...
`agent local-cli` is the quickest way to run local Claude/Codex manually as a Super Node agent:

- creates a new long-lived agent API key
- installs missing Super Node skills into `~/.codex/skills` and `~/.claude/skills`
...
`paperclipai run` without a subcommand still bootstraps and starts a local Super Node instance. The subcommands below inspect and control API heartbeat runs.
...
Prompt handoff creates Super Node work. It does not create a chat session.
...
Required Super Node runtime skills (heartbeat, etc.) remain server-enforced and
are added on top of whatever the desired set names.

### Catalog (app-shipped skills)

The Super Node app ships a curated catalog under `@paperclipai/skills-catalog`.
...
`skills agent clear` sends an empty desired list. Required Super Node skills are
...
- `--request-approval-on-forbidden` turns a 403 install denial into a linked
  ...
  `--approval-issue-id <id>` to attach it to a specific issue. During Super Node
...
provider-owned references without copying the secret value into Super Node.
...
bootstrap credentials in Super Node secrets.
...
Local Super Node data lives under the selected instance root. `PAPERCLIP_HOME` chooses the home directory and `PAPERCLIP_INSTANCE_ID` chooses the instance.
