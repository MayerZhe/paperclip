# Super Node — Product Definition

## What It Is

Super Node is the control plane for autonomous AI companies. One instance of Super Node can run multiple companies. A **company** is a first-order object.
...
- **Adapter type + config** — how this agent runs and what defines its identity/behavior. This is adapter-specific (e.g., an OpenClaw agent might use SOUL.md and HEARTBEAT.md files; a Claude Code agent might use CLAUDE.md; a bare script might use CLI args). Super Node doesn't prescribe the format — the adapter does.
...
Super Node supports several ways to run an agent's heartbeat:

1. **Local CLI/session adapters** — Super Node starts or resumes local coding-tool sessions such as Claude Code, Codex, Gemini, OpenCode, Pi, and Cursor, then tracks the run.
2. **Run a command** — Super Node kicks off a process (shell command, Python script, etc.) and tracks it. The heartbeat is "execute this and monitor it."
3. **Fire and forget a request** — Super Node sends a webhook/API call to an externally running agent. The heartbeat is "notify this agent to wake up." OpenClaw-style hooks work this way.
4. **External adapter plugins** — Super Node loads adapter packages through the plugin/adapter flow so self-hosted installs can add runtimes without hardcoding them in core.

Agent runs can use project and execution workspaces, managed runtime services such as preview/dev servers, adapter-specific session state, and HTTP/webhook-style execution. We provide sensible defaults, but the adapter is still the boundary: if a runtime can be invoked, observed, and authorized, Super Node can coordinate it.
...
5. **Control plane, not execution plane.** Super Node orchestrates. Agents run wherever they run and phone home.

## User Flow (Dream Scenario)

1. Open Super Node, create a new company
...
There are two runtime modes Super Node must support:
...
Super Node’s core identity is a **control plane for autonomous AI companies**, centered on **companies, org charts, goals, issues/comments, heartbeats, budgets, approvals, and board governance**. The public docs are also explicit about the current boundaries: **tasks/comments are the built-in communication model**, Super Node is **not a chatbot**, and it is **not a code review tool**. The roadmap already points toward **easier onboarding, cloud agents, easier agent configuration, plugins, better docs, and ClipMart/ClipHub-style reusable companies/templates**.

## What Super Node should do vs. not do
...
- Do not build a complete Jira/GitHub replacement. The repo/docs already position Super Node as organization orchestration, not focused on pull-request review.
- Do not build enterprise-grade RBAC first. Super Node now has authenticated mode, company memberships, instance roles, and permission grants, but fine-grained enterprise governance should remain secondary to the core company control plane.
