...
ClipHub is the public registry where people share, discover, and download Super Node company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Super Node what a package registry is to a programming language. Super Node already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Super Node — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Super Node instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Super Node spec. It contains:
...
```
Downloads the template and creates a new company in your local Super Node instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Super Node, export your company as a template, then publish:
...
ClipHub is a **separate service** from Super Node itself. Super Node is self-hosted; ClipHub is a hosted registry that Super Node instances talk to.
...
| **Super Node CLI** | `paperclipai install`, `paperclipai publish`, `paperclipai cliphub sync` — built into Super Node |
| **Super Node UI** | "Browse ClipHub" panel in the Super Node web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Super Node) |
| Backend | TypeScript + Hono (consistent with Super Node) |
| Database | PostgreSQL |
...
5. Super Node creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Super Node until it works well
...
## Relationship to Super Node

ClipHub is **not required** to use Super Node. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Super Node what a package registry is to a language runtime: optional, but transformative.
...
- [ ] "Browse ClipHub" panel in Super Node web UI
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Super Node's own philosophy)
