...
This document summarizes the memory systems referenced in task `PAP-530` and extracts the design patterns that matter for Super Node.

## What Super Node Needs From This Survey

Super Node is not trying to become a single opinionated memory engine. The more useful target is a control-plane memory surface that:

- stays company-scoped
- lets each company choose a default memory provider
- lets specific agents override that default
- keeps provenance back to Super Node runs, issues, comments, and documents
...
The question is not "which memory project wins?" The question is "what is the smallest Super Node contract that can sit above several very different memory systems without flattening away the useful differences?"
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because Super Node is local-first today and needs at least one zero-config path.

## Per-Project Notes

| Project | Shape | Notable API / model | Strong fit for Super Node | Main mismatch |
|---|---|---|---|---|
...
| [mem0](https://github.com/mem0ai/mem0) | hosted + OSS SDK | `add`, `search`, `getAll`, `get`, `update`, `delete`, `deleteAll`; entity partitioning via `user_id`, `agent_id`, `run_id`, `app_id` | closest to a clean provider API with identities and metadata filters | provider owns extraction heavily; Super Node should not assume every backend behaves like mem0 |
| [AWS Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html) | AWS-managed memory service | explicit short-term and long-term memories, actor/session/event APIs, memory strategies, namespace templates, optional self-managed extraction pipeline | strong example of provider-managed memory with clear scoped ids, retention controls, and standalone API access outside a single agent framework | AWS-hosted and IAM-centric; Super Node would still need its own company/run/comment provenance, cost rollups, and likely a plugin wrapper instead of baking AWS semantics into core |
| [MemOS](https://github.com/MemTensor/MemOS) | memory OS / framework | unified add-retrieve-edit-delete, memory cubes, multimodal memory, tool memory, async scheduler, feedback/correction | strong source for optional capabilities beyond plain search | much broader than the minimal contract Super Node should standardize first |
...
If Super Node does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why Super Node needs a layered contract instead of a single hard-coded engine.
...
Super Node should support both:

- provider-managed extraction
- Super Node-managed extraction with provider-managed storage / retrieval
...
Super Node should not require a single storage shape. It should require normalized references back to Super Node entities.
...
Super Node should make plain search the minimum contract and richer outputs optional capabilities.
...
Super Node needs both direct request/response operations and background maintenance hooks.

## Super Node-Specific Takeaways

### Super Node should own these concerns

- binding a provider to a company and optionally overriding it per agent
- mapping Super Node entities into provider scopes
- provenance back to issue comments, documents, runs, and activity
- cost / token / latency reporting for memory work
- browse and inspect surfaces in the Super Node UI
...
Super Node does not need to standardize every feature from every provider. It needs:
...
Super Node should adopt a two-layer memory model:

1. `Memory binding + control plane layer`
   Super Node decides which provider key is in effect for a company, agent, or project, and it logs every memory operation with provenance and usage.

2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns Super Node memory requests into provider-specific calls.
...
without forcing Super Node itself to become a monolithic memory engine.
