...
Super Node ships core trust preset names so containment decisions are enforced in
...
scope for risky access, Super Node fails closed.
...
agent can read or mutate through the Super Node API and prevents raw untrusted
output from being automatically promoted into higher-trust agent context.

## Runtime Containment

Managed `low_trust_review` runs fail closed unless Super Node can enforce the
runtime boundary:
...
local review, but Super Node-managed low-trust execution requires a sandboxed
