...
Super Node uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active Super Node config/instance.
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. Super Node uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
If your hosted database requires transaction-pooling-only connections, use a direct or session-pooled connection for Super Node until runtime pooling support is documented in this guide. Do not edit database client source files as part of deployment setup.
...
Super Node stores current-user sidebar membership state in:
...
Super Node supports automatic and manual logical database backups. These dumps include
...
Super Node stores secret metadata and versions in:
