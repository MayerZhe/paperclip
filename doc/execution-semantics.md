...
This document explains how Super Node interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

Super Node separates four concepts that are easy to blur together:
...
This is a hard invariant. Super Node is single-assignee by design.

## 3. Status Semantics

Super Node issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, Super Node may still need a wake path to ensure the assignee actually sees it
...
- waiting on an external dependency or system when Super Node does not own a scheduled re-check
...
- Super Node can wake the assignee
- Super Node can track runs linked to the issue
- Super Node can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- Super Node can track the ownership and status
- Super Node cannot rely on heartbeat/run semantics to keep them moving
...
Super Node already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

Super Node uses two different relationships for different jobs.
...
Blocked issues should stay idle while blockers remain unresolved. Super Node should not create a queued heartbeat run for that issue until the final blocker is done and the `issue_blockers_resolved` wake can start real work.
...
Super Node must treat accepted-plan decomposition as an exact-once control-plane primitive, not as a free-floating wake that any later run may interpret again.
...
Super Node does not need to mandate a specific storage shape in this document. The record may live in a dedicated table, source-issue execution state, interaction metadata, or another durable product surface. What matters is the contract:
...
If the live run disappears, Super Node must repair, resume, or visibly block the existing claim. It must not leave the source issue in a state where a second run can interpret the same acceptance as fresh permission to create sibling issues again.
...
For agent-owned, non-terminal issues, Super Node should never leave work in a state where nobody is responsible for the next move and nothing will wake or surface it.

This is a visibility contract, not an auto-completion contract. If Super Node cannot safely infer the next action, it should surface the ambiguity with a blocked state, a visible notice, or an explicit recovery action. It must not silently mark work done from prose comments or guess that a dependency is complete.
...
For adapter-backed execution, an active run or queued wake counts as a live path only when Super Node can also prove that the selected workspace is coherent for that adapter invocation. A wake that cannot start in the intended workspace is only a failed delivery attempt, not a healthy liveness path.
...
- the adapter will receive the same effective workspace/cwd that Super Node resolved for the run, including the same workspace ids and `PAPERCLIP_WORKSPACE_*` environment values
...
Assigning an issue normally implies executable intent. When create APIs receive an assignee and no explicit status, Super Node defaults the issue to `todo` so the assignee has a wake path instead of silently inheriting the unassigned `backlog` default.

An explicit assigned `backlog` issue remains valid when the creator is deliberately parking the work. It must not wake the assignee just because it has an assignee. Super Node should make that choice visible in activity and UI so operators can distinguish intentional parking from a missed handoff.
...
An `in_review` issue is stalled when it has no typed participant, no pending interaction or approval, no user owner, no active monitor, no active run, no queued wake, and no explicit recovery action. Super Node should surface that state as recovery work rather than silently completing the issue or leaving blocker chains parked indefinitely.
...
- `nextCheckAt`: when Super Node should wake the assignee
- `notes`: non-secret instructions for what the assignee should check
- `serviceName`: optional non-secret external-service context
- `externalRef`: optional external-service reference input; Super Node treats it as secret-adjacent, redacts it before persistence/visibility, and omits it from activity and wake payloads
- `timeoutAt`, `maxAttempts`, and `recoveryPolicy`: optional recovery hints for bounded waits

Monitors are not recurring intervals. When a monitor fires, Super Node clears the scheduled monitor and queues an `issue_monitor_due` wake for the assignee. If the external service is still pending, the assignee must explicitly re-arm the monitor with a new `nextCheckAt`. If the issue moves to `done`, `cancelled`, an invalid status, or a human/unassigned owner, the monitor is cleared.
...
Monitor bounds are enforced. Super Node rejects attempts to re-arm a monitor whose `timeoutAt` or `maxAttempts` is already exhausted. When a scheduled monitor reaches an exhausted bound at trigger time, Super Node clears it and follows `recoveryPolicy`: `wake_owner` queues a bounded recovery wake for the assignee, `create_recovery_issue` opens visible issue-backed recovery work, and `escalate_to_board` records a board-visible escalation comment/activity.

Use `blocked` instead of a monitor when no Super Node assignee owns a responsible polling path. In that case, name the external owner/action or create first-class recovery/blocker work.
...
Super Node now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, Super Node queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, Super Node moves the issue to `blocked` and opens or updates an explicit recovery action when a bounded owner/action is known; the visible comment is evidence, not the recovery path by itself
...
- Super Node queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, Super Node moves the issue to `blocked` and opens or updates an explicit recovery action when a bounded owner/action is known; the visible comment is evidence, not the recovery path by itself

This is an active-work continuity recovery.

### 9.3 Recovery model-profile lane

Cheap model profiles are only for status-only operational recovery overhead. Super Node may request `modelProfile: "cheap"` for bounded recovery-owner work that updates task liveness, clears bad status, records a disposition, or asks for human/manager intervention. Those wakes must carry guard context such as `allowDeliverableWork: false`, `allowDocumentUpdates: false`, and `resumeRequiresNormalModel: true`.
...
On startup and on the periodic recovery loop, Super Node now does five things in sequence:
...
An active run can still be unhealthy even when its process is `running`. Super Node treats prolonged output silence as a watchdog signal, not as proof that the run is failed.
...
Super Node opens an explicit recovery action when the system can identify a problem but cannot safely complete the work itself.
...
In these cases Super Node should leave a visible issue/comment trail instead of silently retrying.

## 13. What This Does Not Mean

These semantics do not change V1 into an auto-reassignment system.

Super Node still does not:
...
That is the execution contract Super Node should present to operators.
