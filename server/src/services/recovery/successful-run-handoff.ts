...
export const SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY =
  "Super Node could not resolve this issue's missing disposition automatically. The issue is blocked on a recovery owner.";
...
export function buildSuccessfulRunHandoffInstruction(input: {
  ...
  return [
    `Your previous run on ${issueLabel} succeeded, but the issue is still in \`in_progress\` and Super Node cannot identify a valid issue disposition.`,
    "",
    "Resolve the missing disposition before creating or revising any new artifacts. Choose **exactly one** outcome and perform the matching Super Node action:",
    "",
