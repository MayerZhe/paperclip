...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "Super Node coordination skill",
        markdown: "---\nname: paperclip\ndescription: Super Node coordination skill\n---\n\n# Super Node\n",
        sourceType: "github",
  ...
  it("exports referenced skills as stubs by default with sanitized Super Node extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Super Node"');
  ...
  it("exports default sidebar order into the Super Node extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("# Super Node");
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/references/api.md"])).toContain("# API");
  });

  it("exports catalog skill provenance in portable Super Node frontmatter", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Super Node",
      },
  ...
  it("normalizes invalid imported project icon names to null", async () => {
    ...
    await portability.importBundle({
      ...
      target: { mode: "new_company", newCompanyName: "Imported Super Node" },
      collisionStrategy: "rename",
    }, "user-1");
  ...
  it("exports routines as recurring task packages with Super Node routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Super Node"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .paperclip.yaml", async () => {
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Super Node");
