...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Super Node Agent Onboarding");
    ...
    expect(text).toContain("Suggested Super Node base URLs to try");
    ...
    expect(text).toContain("Decide which Super Node adapter type matches your runtime.");
  });
