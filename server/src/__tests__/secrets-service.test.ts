...
describeEmbeddedPostgres("secretService", () => {
  ...
  it("sanitizes AWS remote import preview provider errors before crossing the service boundary", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Super-Node is not authorized to perform secretsmanager:ListSecrets";
  ...
  it("sanitizes AWS remote import row provider errors", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Super-Node is not authorized to perform secretsmanager:DescribeSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
  ...
  it("rejects Super Node-managed AWS namespace refs during preview and import commit", async () => {
    const companyId = await seedCompany();
    const svc = secretService(db);
    const awsVault = await svc.createProviderConfig(companyId, {
      provider: "aws_secrets_manager",
    ...
    expect(result.results[0]?.reason).toMatch(/Super Node-managed namespace/i);
