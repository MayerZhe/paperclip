...
/** Resolve the Super Node repo skills directory (built-in / managed skills). */
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  // Build set of Super Node-managed skill names
  ...
  if (paperclipSkillsDir) {
    ...
    } catch { /* skip Super Node skills directory */ }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Super Node host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Super Node is bound to loopback in authenticated/private mode.",
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an external Super Node agent, save your one-time claim secret, wait for board approval, then claim your API key. Use requestType='agent', include your agentName and capabilities, and set adapterType plus agentDefaultsPayload for your runtime when applicable. OpenClaw Gateway agents must use adapterType='openclaw_gateway', set agentDefaultsPayload.url to a ws:// or wss:// gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token.",
      inviteMessage: extractInviteMessage(invite),
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Super Node hostname is reachable and allowed via `pnpm paperclipai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Super Node API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific Super Node skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    The board approves the join request in Super Node before key claim is allowed.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set PAPERCLIP_API_KEY and PAPERCLIP_API_URL for future Super Node API calls.
    ...
    ## Step 4: Install Super Node skill
    ...
    ${
      ...
      "Ensure Super Node is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    ...
    appendBlock(`
      ...
      - then restart Super Node and retry onboarding.
    `);
  }
