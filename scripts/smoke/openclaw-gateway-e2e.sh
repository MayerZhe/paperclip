...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set PAPERCLIP_API_KEY=${AGENT_API_KEY} in your run context before running Super Node heartbeat steps."
...
main() {
  ...
  wait_http_ready "${PAPERCLIP_API_URL%/}/api/health" 15 || fail "Super Node API health endpoint not reachable"
