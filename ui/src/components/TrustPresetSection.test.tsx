    ...
    createdRoot.render(
      <TooltipProvider>
        <TrustPresetSection
          ...
          onChange={onChange}
          companyId="company-1"
          projectCandidates={[{ id: "project-1", label: "Super Node App" }]}
...
describe("TrustPresetSection", () => {
  it("hides the CE boundary editor under Standard", () => {
    ...
    expect(view.text()).not.toContain("Get Super Node EE.");
  });
  ...
  it("shows a selectable CE boundary editor for low-trust review", () => {
    ...
    expect(view.text()).toContain("Super Node App");
    expect(view.text()).toContain("Get Super Node EE.");
