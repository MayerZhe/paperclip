...
const NATIVE_HIGHLIGHT_NAMES: Record<NativeHighlightKind, string> = {
  open: "super-node-doc-annotation-open",
  focused: "super-node-doc-annotation-focused",
  stale: "super-node-doc-annotation-stale",
  resolved: "super-node-doc-annotation-resolved",
};
    ...
    const mutationObserver = typeof window.MutationObserver === "function" && container
      ? new window.MutationObserver((mutations) => {
        const onlyLayerMutations = mutations.every((mutation) => {
          ...
          return !!target?.closest(".super-node-doc-annotation-layer, .super-node-doc-annotation-visual-layer");
        });
  ...
  return (
    <>
      {!nativeHighlightsSupported ? (
        <div className="super-node-doc-annotation-visual-layer pointer-events-none absolute inset-0 z-0" aria-hidden="true">
            ...
            {highlightRects.map((rect, index) => {
              ...
              return (
                <span
                  ...
                  className={cn(
                    "super-node-doc-annotation-highlight absolute rounded-none transition-colors",
      ...
      <div
        className="super-node-doc-annotation-layer pointer-events-none absolute inset-0 z-[2]"
        aria-hidden="true"
      >
        <div ref={overlayRef} className="relative h-full w-full">
          {highlightRects.map((rect, index) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "super-node-doc-annotation-hit-target pointer-events-auto absolute cursor-pointer rounded-none bg-transparent",
          ...
          {highlightRects.map((rect, index) =>
            rect.isTail && rect.anchorState === "stale" ? (
              <span
                ...
                aria-hidden="true"
                data-thread-id={rect.threadId}
                className="super-node-doc-annotation-tail pointer-events-none absolute inline-flex items-center justify-center rounded-sm bg-amber-500/95 text-amber-50 shadow-sm dark:bg-amber-500/90 dark:text-amber-50"
                style={{
          ...
          {pendingAnchor && toolbarPosition ? (
            <div
              ...
              className="super-node-doc-annotation-selection-toolbar pointer-events-auto absolute z-10 flex items-center gap-1 rounded-md border border-border bg-popover px-1 py-1 shadow-md"
