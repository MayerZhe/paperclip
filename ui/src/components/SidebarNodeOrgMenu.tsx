...
import { useNodeOrgOrder } from "@/hooks/useNodeOrgOrder";
import { queryKeys } from "@/lib/queryKeys";
import { cn } from "@/lib/utils";
import { useSidebar } from "../context/SidebarContext";
import { NodeOrgPatternIcon } from "./NodeOrgPatternIcon";

interface SidebarNodeOrgMenuProps {
  open?: boolean;
...
export function SidebarNodeOrgMenu({ open: controlledOpen, onOpenChange }: SidebarNodeOrgMenuProps = {}) {
  const [internalOpen, setInternalOpen] = useState(false);
  const [isEditingOrder, setIsEditingOrder] = useState(false);
  const queryClient = useQueryClient();
  const { companies, selectedCompany, setSelectedCompanyId } = useNodeOrg();
  ...
  const { orderedCompanies, persistOrder } = useNodeOrgOrder({
