  ...
  const restoreMutation = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : "Super Node could not restore the revision.",
        tone: "error",
      });
    },
  });
    ...
    <div className="space-y-4">
      ...
      <div className={`${cardWrapper} p-3 space-y-2`}>
        ...
        <p className="text-xs text-muted-foreground">
          ...
          Super Node mints fresh secret material at restore time.
        </p>
      </div>
        ...
        <ul className="space-y-2 text-sm">
          ...
          {recreatedWebhookLabels.map((label) => (
            <li key={label} className="flex items-start gap-2 text-amber-200">
              ...
              The webhook trigger {label} will be recreated with a new URL and secret. Super Node will
