...
describe("CloudAccessGate", () => {
  ...
  it("shows a no-access message for signed-in users without org access", async () => {
    ...
    await waitForText(container, "No node org access");

    expect(container.textContent).toContain("No node org access");
  ...
  it("allows authenticated users with company access through to the board", async () => {
    ...
    expect(container.textContent).not.toContain("No node org access");
  ...
  it("shows browser sign-in setup for signed-out private bootstrap-pending instances", async () => {
    ...
    const root = renderGate(container);
    await waitForText(container, "Finish setting up this Super Node");

    expect(container.textContent).toContain("Finish setting up this Super Node");
  ...
  it("keeps public bootstrap-pending instances invite-only", async () => {
    ...
    const root = renderGate(container);
    await waitForText(container, "This Super Node is waiting on its first admin");

    expect(container.textContent).toContain("This Super Node is waiting on its first admin");
