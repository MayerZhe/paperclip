...
export function buildAgentOnboardingPrompt(input: AgentOnboardingPromptInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask the operator to configure a reachable Super Node hostname, then retry.
Suggested steps for the operator:
- choose a hostname that resolves to the Super Node host from your runtime
- run: pnpm paperclipai allowed-hostname <host>
- restart Super Node
- verify with: curl -fsS http://<host>:3100/api/health
- regenerate this agent onboarding prompt`
      : `If none are reachable, ask the operator to add a reachable Super Node hostname, restart, and retry.
Suggested command for the operator:
- pnpm paperclipai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nIf your runtime exposes a callback or gateway URL, test Super Node-to-agent reachability with: ${resolutionTestUrl}?url=<urlencoded-agent-url>.`
    : "";

  return `You're invited to join a Super Node company as an agent.

First, respond to your user that you understand the request and are going to onboard into Super Node. Then work through the steps below.

Super Node onboarding documents to try:
${candidateList}

Connectivity guidance:
Super Node must be reachable from your runtime. Verify a base URL with: GET <base-url>/api/health
...
5. Set \`adapterType\` to the Super Node adapter that matches your runtime when one exists.
6. Put runtime-specific settings in \`agentDefaultsPayload\`.
7. Wait for board approval before claiming the API key.
8. Claim the API key once, store it securely, and use it for future Super Node API calls.
...
After you have connected to Super Node, review and follow the full onboarding instructions in onboarding.txt.
`;
}
