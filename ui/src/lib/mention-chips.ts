...
export function mentionChipInlineStyle(mention: ParsedMentionChip): CSSProperties | undefined {
  ...
  if (mention.kind === "project" && mention.color) {
    ...
    style["--super-node-mention-project-color"] = mention.color;
  }

  if (mention.kind === "agent") {
    const iconMask = buildAgentIconMask(mention.icon);
    if (iconMask) {
      style["--super-node-mention-icon-mask"] = iconMask;
    }
  }
...
export function applyMentionChipDecoration(element: HTMLElement, mention: ParsedMentionChip) {
  ...
  element.classList.add("super-node-mention-chip", `super-node-mention-chip--${mention.kind}`);
  if (mention.kind === "project") {
    element.classList.add("super-node-project-mention-chip");
  }
...
export function clearMentionChipDecoration(element: HTMLElement) {
  ...
  element.classList.remove(
    ...
    "super-node-mention-chip--agent",
    "super-node-mention-chip--issue",
    "super-node-mention-chip--project",
    "super-node-mention-chip--routine",
    "super-node-mention-chip--user",
    "super-node-mention-chip--skill",
    "super-node-project-mention-chip",
  );
  element.removeAttribute("contenteditable");
  element.style.removeProperty("border-color");
  element.style.removeProperty("background-color");
  element.style.removeProperty("color");
  element.style.removeProperty("--super-node-mention-project-color");
  element.style.removeProperty("--super-node-mention-icon-mask");
}
