...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Super Node", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Super Node",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Super Node");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Super Node");
    expect(buildDocumentTitle([], null)).toBe("Super Node");
  });
});
