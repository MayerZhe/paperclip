...
import type { CompanySelectionSource } from "../lib/node-org-selection";
type CompanySelectionOptions = { source?: CompanySelectionSource };

interface NodeOrgContextValue {
  companies: Company[];
...
const STORAGE_KEY = "super-node.selectedNodeOrgId";

const NodeOrgContext = createContext<NodeOrgContextValue | null>(null);
...
export function NodeOrgProvider({ children }: { children: ReactNode }) {
  ...
  return <NodeOrgContext.Provider value={value}>{children}</NodeOrgContext.Provider>;
}

export function useNodeOrg() {
  const ctx = useContext(NodeOrgContext);
  if (!ctx) {
    throw new Error("useNodeOrg must be used within NodeOrgProvider");
  }
  return ctx;
}

/**
 * Non-throwing variant of {@link useNodeOrg}. Returns null when called outside a
 * NodeOrgProvider instead of throwing, so components that may render in
 * provider-less surfaces (e.g. exported/standalone markdown) can read company
 * state without crashing.
 */
export function useOptionalNodeOrg(): NodeOrgContextValue | null {
  return useContext(NodeOrgContext);
}
