...
import { buildInitialExportCheckedFiles } from "../lib/node-org-export-selection";
import { useAgentOrder } from "../hooks/useAgentOrder";
import { useProjectOrder } from "../hooks/useProjectOrder";
import { buildPortableSidebarOrder } from "../lib/node-org-portability-sidebar";
 ...
 * renderYamlBlock.
 */
function filterSuperNodeYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Node Org](https://paperclip.ing) package.");
  lines.push("");
  ...
  lines.push("```");
  lines.push("");
  lines.push("See [Super Node](https://paperclip.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Super Node](https://paperclip.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function NodeOrgExport() {
  ...
  const effectiveFiles = useMemo(() => {
    ...
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterSuperNodeYaml(exportData.files[yamlPath], checkedFiles);
    }

    // Regenerate README.md based on checked selection
    if (typeof exportData.files["README.md"] === "string") {
      const companyName = exportData.manifest.company?.name ?? selectedCompany?.name ?? "Node Org";
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message="Select a Node Org to export." />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <span className="font-medium">
              {selectedCompany?.name ?? "Node Org"} export
            </span>
