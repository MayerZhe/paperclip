...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error("Select a .zip node org package.");
  }
...
export function NodeOrgImport() {
  ...
  const localZipHelpText =
    "Upload a .zip exported directly from Super Node. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly.";
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Download} message="Select a Node Org to import into." />;
  }
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Choose a GitHub repo or upload a local Super Node zip package.
          </p>
        </div>
        ...
        <Field label="Target" hint="Import into this node org or create a new one.">
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
            onChange={(e) => {
          ...
          >
            <option value="new">Create New Node Org</option>
            <option value="existing">
              Existing node org: {selectedCompany?.name}
            </option>
          </select>
        </Field>
        ...
        <Field
          ...
          hint="Board imports can rename, skip, or replace matching node org content."
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
