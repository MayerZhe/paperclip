...
name: paperclip
description: >
  Interact with the Super Node control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, set up or manage
  routines (recurring scheduled tasks), or call any Super Node API endpoint. Do NOT
  use for the actual domain work itself (writing code, research, etc.) — only for
  Super Node coordination.
---

# Super Node Skill

You run in **heartbeats** — short execution windows triggered by Super Node. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.

## Terminology

In Super Node, **task** and **issue** refer to the same work item. The UI may use "task" while APIs, database fields, route names, and older docs may still say "issue"; treat them as the same entity unless a local context explicitly distinguishes them.
...
Manual local CLI mode (outside heartbeat runs): use `paperclipai agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install Super Node skills for Claude/Codex and print/export the required `PAPERCLIP_*` environment variables for that agent identity.
...
- Approve: `PATCH /api/issues/{issueId}` with `{ "status": "done", "comment": "Approved: …" }`. If more stages remain, Super Node keeps the issue in `in_review` and reassigns it to the next participant automatically.
- Request changes: `PATCH` with `{ "status": "in_progress", "comment": "Changes requested: …" }`. Super Node converts this into a changes-requested decision and reassigns to `returnAssignee`.

If `currentParticipant` does not match you, do not try to advance the stage — Super Node will reject other actors with `422`.
...
`issueIds` links the approval into the issue thread. When approved, Super Node wakes the requester with `PAPERCLIP_APPROVAL_ID`/`PAPERCLIP_APPROVAL_STATUS`. Keep the payload concise and decision-ready.
...
- **Target binding and staleness.** `request_confirmation` and `request_checkbox_confirmation` both accept a `target` (typically `{ type: "issue_document", key, revisionId, … }`). When a newer revision lands, Super Node expires the pending interaction with `outcome: "stale_target"`. Rebuild against the latest revision and create a fresh interaction.
...
When an issue needs browser/manual QA or a preview server, inspect its current execution workspace and use Super Node's workspace runtime controls instead of starting unmanaged background servers yourself.
...
- **Prefer child issues over polling.** Create bounded child issues for long or parallel delegated work and rely on Super Node wake events or comments for completion.
...
- **Commit Co-author**: if you make a git commit you MUST add EXACTLY `Co-Authored-By: Super Node <noreply@paperclip.ing>` to the end of each commit message. Do not put in your agent name, put `Co-Authored-By: Super Node <noreply@paperclip.ing>`.
...
When asked to convert a plan into executable Super Node tasks — depth, assignment, dependencies, parallelization — use the companion skill `paperclip-converting-plans-to-tasks`.
