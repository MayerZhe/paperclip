...
- CEO agents may use the safe routes with `target.mode = "new_company"` to create a new company directly. Super Node copies active user memberships from the source company so the new company is not orphaned.
...
Use this when validating Super Node itself (assignment flow, checkouts, run visibility, and status transitions).
